...
void syncTimeWithNTP() {
  ...
  if (retry < maxRetries) {
    // 设置东八区时区 (CST-8 表示 UTC+8)
    setenv("TZ", "CST-8", 1);
    tzset();
    
    Serial.printf("[%lu] [KOSync] NTP time synced (UTC+8)\n", millis());
#ifdef CROSSPOINT_X3
    // NTP 同步成功后，将系统时间回写到 RTC 芯片
    if (DS3231RTC::syncRTCFromSystem()) {
      Serial.printf("[%lu] [KOSync] RTC updated from NTP\n", millis());
    }
#endif
  } else {
...
void KOReaderSyncActivity::render() {
  ...
  M4UiText::drawCentered(renderer, UI_12_FONT_ID, 15, "KOReader Sync", true, EpdFontFamily::BOLD);

  if (state == NO_CREDENTIALS) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 280, "No credentials configured", true, EpdFontFamily::BOLD);
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 320, "Set up KOReader account in Settings");
  ...
  if (state == SYNCING || state == UPLOADING) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 300, statusMessage.c_str(), true, EpdFontFamily::BOLD);
    renderer.displayBuffer();
    return;
  }
  ...
  if (state == SHOWING_RESULT) {
    // Show comparison
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 120, "Progress found!", true, EpdFontFamily::BOLD);
    ...
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, 160, "Remote:", true);
    char remoteChapterStr[128];
    snprintf(remoteChapterStr, sizeof(remoteChapterStr), "  %s", remoteChapter.c_str());
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, 185, remoteChapterStr);
    char remotePageStr[64];
    snprintf(remotePageStr, sizeof(remotePageStr), "  ~Page %d, %.0f%% overall", remotePosition.pageNumber + 1,
             remoteProgress.percentage * 100);
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, 210, remotePageStr);

    if (!remoteProgress.device.empty()) {
      char deviceStr[64];
      snprintf(deviceStr, sizeof(deviceStr), "  From: %s", remoteProgress.device.c_str());
      M4UiText::draw(renderer, UI_10_FONT_ID, 20, 235, deviceStr);
    }

    // Local progress - chapter and page
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, 270, "Local:", true);
    char localChapterStr[128];
    snprintf(localChapterStr, sizeof(localChapterStr), "  %s", localChapter.c_str());
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, 295, localChapterStr);
    char localPageStr[64];
    snprintf(localPageStr, sizeof(localPageStr), "  Page %d/%d, %.2f%% overall", currentPage + 1, totalPagesInSpine,
             localProgress.percentage * 100);
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, 320, localPageStr);
    ...
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, optionY, "Apply remote progress", selectedOption != 0);

    // Upload option
    if (selectedOption == 1) {
      renderer.fillRect(0, optionY + optionHeight - 2, pageWidth - 1, optionHeight);
    }
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, optionY + optionHeight, "Upload local progress", selectedOption != 1);

    // Cancel option
    if (selectedOption == 2) {
      renderer.fillRect(0, optionY + optionHeight * 2 - 2, pageWidth - 1, optionHeight);
    }
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, optionY + optionHeight * 2, "Cancel", selectedOption != 2);
  ...
  if (state == NO_REMOTE_PROGRESS) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 280, "No remote progress found", true, EpdFontFamily::BOLD);
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 320, "Upload current position?");
  ...
  if (state == UPLOAD_COMPLETE) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 300, "Progress uploaded!", true, EpdFontFamily::BOLD);
  ...
  if (state == SYNC_FAILED) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 280, "Sync failed", true, EpdFontFamily::BOLD);
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, 320, statusMessage.c_str());
...
void KOReaderSyncActivity::loop() {
  ...
  if (state == SHOWING_RESULT) {
    ...
    if (mappedInput.wasPressed(MappedInputManager::Button::Confirm)) {
      if (selectedOption == 0) {
        // Apply remote progress - use percentage for precise positioning
        onSyncComplete(remoteProgress.percentage);
