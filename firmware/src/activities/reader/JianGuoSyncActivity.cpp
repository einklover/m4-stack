...
int JianGuoSyncActivity::lastExtractedChapterPos = 0;
...
if (state == BrowserState::SHOWING_RESULT) {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
    if (selectedOption == 0) {
      // 下载：应用云端进度，计算全书百分比用于精确定位
      float bookPercentage = 0.0f;
      if (epub && epub->getBookSize() > 0) {
        // 通过标题匹配后的章节索引获取spine索引
        int spineIdx = lastExtractedChapterIndex;
        if (spineIdx >= 0 && spineIdx < epub->getSpineItemsCount()) {
          // 计算章节内进度：用durChapterPos估算
          const size_t prevCumSize = (spineIdx > 0) ? epub->getCumulativeSpineItemSize(spineIdx - 1) : 0;
          const size_t curCumSize = epub->getCumulativeSpineItemSize(spineIdx);
          const size_t spineSize = curCumSize - prevCumSize;
          float intraProgress = 0.0f;
          if (spineSize > 0 && lastExtractedChapterPos > 0) {
            // durChapterPos是字符位置，spineSize是字节数
            // 估算：假设中文约3字节/字符，粗略估算章节内进度
            const float estimatedCharsInSpine = static_cast<float>(spineSize) / 3.0f;
            intraProgress = std::min(1.0f, static_cast<float>(lastExtractedChapterPos) / estimatedCharsInSpine);
          }
          bookPercentage = epub->calculateProgress(spineIdx, intraProgress);
        }
      }
      Serial.printf("[%lu] [JG] 选择下载，章节：%d, pos：%d, 百分比：%.1f%%\n", 
                    millis(), lastExtractedChapterIndex, lastExtractedChapterPos, bookPercentage * 100);
      onSelectChapter(bookPercentage);
      onGoBack();
    } else {
      // 上传：上传当前进度
...
void JianGuoSyncActivity::render() const {
  ...
  M4UiText::drawCentered(renderer, UI_12_FONT_ID, 15, "阅读进度同步", true, EpdFontFamily::BOLD);

  // 检查 WiFi
  if (state == BrowserState::CHECK_WIFI) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2, "连接 WiFi 中...");
  ...
  if (state == BrowserState::DOWNLOADING) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 - 30, "下载进度文件...");
  ...
  if (state == BrowserState::UPLOADING) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 - 30, "上传进度文件...");
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2, statusMessage.c_str());
    const auto labels = mappedInput.mapLabels("返回", "", "", "");
    GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
    renderer.displayBuffer();
  ...
  // 显示结果（选择下载/上传）
  if (state == BrowserState::SHOWING_RESULT) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 - 50, "✓ 找到进度文件");
    
    char infoBuf[64];
    snprintf(infoBuf, sizeof(infoBuf), "\u4e91\u7aef\u7ae0\u8282\uff1a%d \u4f4d\u7f6e\uff1a%d", lastExtractedChapterIndex, lastExtractedChapterPos);
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 - 20, infoBuf);
        
    snprintf(infoBuf, sizeof(infoBuf), "\u672c\u5730\u7ae0\u8282\uff1a%d", currentSpineIndex);
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 + 5, infoBuf);

    if (!lastExtractedBookName.empty()) {
        M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 + 35, lastExtractedBookName.c_str());
    }
    ...
    if (selectedOption == 0) {
      ...
      M4UiText::draw(renderer, UI_10_FONT_ID, 20, optionY, "← 下载云端进度", false);
    } else {
      M4UiText::draw(renderer, UI_10_FONT_ID, 20, optionY, "  下载云端进度", true);
    }

    // 上传选项
    if (selectedOption == 1) {
      renderer.fillRect(0, optionY + optionHeight - 2, pageWidth - 1, optionHeight);
      M4UiText::draw(renderer, UI_10_FONT_ID, 20, optionY + optionHeight, "→ 上传本地进度", false);
    } else {
      M4UiText::draw(renderer, UI_10_FONT_ID, 20, optionY + optionHeight, "  上传本地进度", true);
    }
  ...
  // 完成
  if (state == BrowserState::COMPLETE) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 - 30, "✓ 同步完成");
    char infoBuf[64];
    snprintf(infoBuf, sizeof(infoBuf), "章节：%d", lastExtractedChapterIndex);
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2, infoBuf);
    if (!lastExtractedBookName.empty()) {
        M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 + 25, lastExtractedBookName.c_str());
    }
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight - 30, "按返回键退出");
    renderer.displayBuffer();
    return;
  }
  ...
  // 错误
  if (state == BrowserState::ERROR) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 - 20, "失败");
...
void JianGuoSyncActivity::parseAndCleanup() {
  ...
  // === 解析 durChapterPos（章节内字符位置）===
  int durChapterPos = 0;
  String keyPos = "\"durChapterPos\":";
  int posIdx = jsonContent.indexOf(keyPos);
  if (posIdx != -1) {
      int startVal = posIdx + keyPos.length();
      while (startVal < jsonContent.length() && isspace(jsonContent[startVal])) startVal++;
      String numStr = "";
      while (startVal < jsonContent.length() && isdigit(jsonContent[startVal])) {
          numStr += jsonContent[startVal];
      ...
      if (numStr.length() > 0) {
          durChapterPos = numStr.toInt();
      }
  }

  Serial.printf("[JG] 云端章节内位置(durChapterPos)：%d\n", durChapterPos);
...
if (!durChapterTitle.isEmpty() && chapterIndex >= 0) {
  ...
  Serial.printf("[JG] 最终章节索引：%d\n", chapterIndex);
    ...
    lastExtractedChapterPos = durChapterPos;
...
void JianGuoSyncActivity::uploadProgressFile() {
    ...
    if (posPos != -1) {
        ...
        // 估算当前章节内字符位置：基于当前页在章节内的进度
        int newChapterPos = 0;
        // 注意：这里无法获取精确的字符位置，因为我们不知道目标设备的页面信息
        // 使用0作为保守值，待下载时会用百分比定位
        String newJson = jsonContent.substring(0, startVal) + 
                        String(newChapterPos) + 
                        jsonContent.substring(endVal);
        jsonContent = newJson;
        
        Serial.printf("[%lu] [JG] 修改 durChapterPos: %d → %d\n", millis(), 
                      oldChapterPos, newChapterPos);
    }
