...
void OpdsBookBrowserActivity::loop() {
  ...
  auto touchBack = [this]() {
    return mappedInput.hasTouch() && mappedInput.wasBackGesture();
  };
  ...
  if (state == BrowserState::ERROR) {
    ...
    if (mappedInput.hasTouch() && mappedInput.wasScreenTapped(tx, ty)) {
      // Full-screen confirm band: any tap retries (same as Confirm)
  ...
  // Handle downloading state - no interactive controls (progress only)
  ...
  if (state == BrowserState::BROWSING) {
    // Touch list: geometry matches render() (startY=60, rowStep=30, PAGE_ITEMS)
    if (mappedInput.hasTouch()) {
      ...
      layout.listHeight = PAGE_ITEMS * rowStep;
      layout.rowStep = rowStep;
      layout.itemCount = itemCount;
      layout.selectedIndex = selectorIndex;
      layout.maxVisible = 0;  // paged list via listIndexFromPoint
      int hit = -1;
      ...
      if (act == M4ListTouchPolicy::Action::PageDown || act == M4ListTouchPolicy::Action::PageUp) {
        if (itemCount > 0) {
          selectorIndex = M4ListTouchPolicy::applyPage(selectorIndex, itemCount, PAGE_ITEMS,
      ...
      if (act == M4ListTouchPolicy::Action::Activate && hit >= 0) {
        ...
        if (entry.type == OpdsEntryType::BOOK) downloadBook(entry);
        else navigateToEntry(entry);
        return;
      }
    }
...
void OpdsBookBrowserActivity::render() const {
  ...
  M4UiText::drawCentered(renderer, UI_12_FONT_ID, 15, "OPDS Browser", true, EpdFontFamily::BOLD);
  ...
  if (state == BrowserState::ERROR) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 - 20, "Error:");
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2 + 10, errorMessage.c_str());
    const auto labels = mappedInput.mapLabels(L(Str::kBack), L(Str::kRetry), "", "");
    GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
    renderer.displayBuffer();
    return;
  }
  ...
  const auto labels = mappedInput.mapLabels(L(Str::kBack), confirmLabel, "", "");
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);

  if (entries.empty()) {
    M4UiText::drawCentered(renderer, UI_10_FONT_ID, pageHeight / 2, "No entries found");
    renderer.displayBuffer();
    return;
  }
  ...
  for (size_t i = pageStartIndex; i < entries.size() && i < static_cast<size_t>(pageStartIndex + PAGE_ITEMS); i++) {
    ...
    auto item = M4UiText::truncated(renderer, UI_10_FONT_ID, displayText.c_str(), renderer.getScreenWidth() - 40);
    M4UiText::draw(renderer, UI_10_FONT_ID, 20, 60 + (i % PAGE_ITEMS) * 30, item.c_str(),
...
void OpdsBookBrowserActivity::downloadBook(const OpdsEntry& book) {
  ...
  if (result == HttpDownloader::OK) {
    ...
    state = BrowserState::ERROR;
    errorMessage = L(Str::kDownloadFailedShort);
    updateRequired = true;
  }
}
