...
class KeyboardEntryActivity : public Activity {
 ...
 private:
  ...
  // Row 0 control buttons: [扫码输入] [确认] [退格] [shift] [空格]
  static constexpr int ROW0_QR_COL = 0;
  static constexpr int ROW0_OK_COL = 1;
  static constexpr int ROW0_BACKSPACE_COL = 2;
  static constexpr int ROW0_SHIFT_COL = 3;
  static constexpr int ROW0_SPACE_COL = 4;

  // Drawn key rows: row0 controls + rows 1..4 chars (not NUM_ROWS which is array size).
  static constexpr int VISIBLE_KEY_ROWS = 5;
  ...
  KeyMetrics computeKeyMetrics(int pageWidth, int pageHeight, int inputEndY) const;
  TouchHitGeometry::KeyboardLayout buildHitLayout(const KeyMetrics& m) const;
  void render() const;
  void renderQRScreen() const;
  void renderKeyBox(int boxX, int boxY, int boxW, int boxH, const char* label, bool isSelected) const;
