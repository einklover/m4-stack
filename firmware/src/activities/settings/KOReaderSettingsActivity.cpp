...
void KOReaderSettingsActivity::render() {
  ...
  // Draw header
  M4UiText::drawCentered(renderer, UI_12_FONT_ID, 15, L(Str::kKOReaderSync), true, EpdFontFamily::BOLD);

  // Draw selection highlight
  renderer.fillRect(0, 60 + selectedIndex * 40 - 2, pageWidth - 1, 40);

  // Draw menu items
  for (int i = 0; i < MENU_ITEMS; i++) {
    const int settingY = 60 + i * 40;
    ...
    const char* status = "";
    if (i == 0) {
      status = KOREADER_STORE.getUsername().empty() ? L(Str::kNotSet) : L(Str::kHasBeenSet);
    } else if (i == 1) {
      status = KOREADER_STORE.getPassword().empty() ? L(Str::kNotSet) : L(Str::kHasBeenSet);
    } else if (i == 2) {
      status = KOREADER_STORE.getServerUrl().empty() ? L(Str::kDefault) : L(Str::kCustom);
    } else if (i == 3) {
      status = KOREADER_STORE.getMatchMethod() == DocumentMatchMethod::FILENAME ? "[Filename]" : "[Binary]";
    } else if (i == 4) {
      status = KOREADER_STORE.hasCredentials() ? "" : L(Str::kPleaseSetCredentials);
    }
