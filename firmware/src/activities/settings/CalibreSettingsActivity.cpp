...
void CalibreSettingsActivity::render() {
  ...
  // Draw header
  M4UiText::drawCentered(renderer, UI_12_FONT_ID, 15, L(Str::kOPDSBrowser), true, EpdFontFamily::BOLD);

  // Draw info text about Calibre
  M4UiText::drawCentered(renderer, UI_10_FONT_ID, 40, L(Str::kConfigCalibreOpds), false);
  ...
  for (int i = 0; i < MENU_ITEMS; i++) {
    ...
    if (i == 0) {
      status = (strlen(SETTINGS.opdsServerUrl) > 0) ? L(Str::kConfigured) : L(Str::kNotConfigured);
    } else if (i == 1) {
      status = (strlen(SETTINGS.opdsUsername) > 0) ? L(Str::kConfigured) : L(Str::kNotConfigured);
    } else if (i == 2) {
      status = (strlen(SETTINGS.opdsPassword) > 0) ? L(Str::kConfigured) : L(Str::kNotConfigured);
    }
