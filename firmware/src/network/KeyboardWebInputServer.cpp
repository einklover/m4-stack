...
void KeyboardWebInputServer::stop() {
  ...
  if (apModeStarted) {
    ...
    WiFi.mode(WIFI_OFF);
    delay(300);
    // 注意：Arduino WiFi 库已经处理了 esp_wifi_stop()
    // 无法直接调用 esp_wifi_deinit()，因为 Arduino 框架假设 WiFi 会被重用
    apModeStarted = false;
...
void KeyboardWebInputServer::handleRootPage() {
  server->send_P(200, "text/html", TextInputPageHtml);
