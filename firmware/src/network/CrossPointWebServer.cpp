    ...
    if (packetSize > 0) {
      ...
      if (len > 0) {
        ...
        if (strcmp(buffer, "hello") == 0) {
          // Discovery pings almost always come from the phone that will also
          // host Legado web — record the peer as a visitor seed.
          noteTransferVisitorIp(udp.remoteIP());
...
void CrossPointWebServer::handleFileList() const {
  noteTransferVisitor(server.get());
  server->send_P(200, "text/html", FilesPageHtml);
}
