...
#pragma once
constexpr char SettingsPageHtml[] PROGMEM = R"rawliteral(<!DOCTYPE html><html><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>CrossLink - 设置</title> <style>
    body {
  ...
  </style> </head><body><h1>⚙️ 设置</h1><div class="nav-links"><a href="/">首页</a><a href="/files">文件管理</a><a href="/settings">设置</a></div><div id="message" class="message"></div><div id="settings-container"><div class="loader-container"><span class="loader"></span></div></div><div class="save-container" id="save-container" style="display:none;"><button class="save-btn" id="saveBtn" onclick="saveSettings()">保存设置</button></div><div class="card"><p style="text-align: center; color: #95a5a6; margin: 0;"> CrossLink 电子书阅读器 </p></div> <script>
  let allSettings = [];
  let originalValues = {};
  ...
  async function loadSettings() {
    try {
      ...
      document.getElementById('settings-container').innerHTML =
        '<div class="card"><p style="text-align:center;color:#e74c3c;">加载设置失败</p></div>';
    }
  }
