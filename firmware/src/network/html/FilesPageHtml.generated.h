...
#pragma once
constexpr char FilesPageHtml[] PROGMEM = R"rawliteral(<!DOCTYPE html><html><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>CrossLink - 文件管理</title> <style>
    body {
  ...
  async function hydrate() {
    ...
    try {
      ...
      console.error(e);
      fileTable.innerHTML = '<div class="no-files">加载文件时发生错误</div>';
      return;
    }
    ...
    document.getElementById('folder-summary').innerHTML = `${folderCount} 个文件夹，${files.length - folderCount} 个文件，${formatFileSize(totalSize)}`;

    if (files.length === 0) {
      fileTable.innerHTML = '<div class="no-files">此文件夹为空</div>';
    } else {
      let fileTableContent = '<table class="file-table">';
      fileTableContent += '<tr><th>名称</th><th>类型</th><th>大小</th><th class="actions-col">操作</th></tr>';
      ...
      sortedFiles.forEach(file => {
        if (file.isDirectory) {
          ...
          fileTableContent += `<td><span class="file-icon">📁</span><a href="/files?path=${encodeURIComponent(folderPath)}" class="folder-link">${escapeHtml(file.name)}</a><span class="folder-badge">文件夹</span></td>`;
          fileTableContent += '<td>文件夹</td>';
          fileTableContent += '<td>-</td>';
          fileTableContent += `<td class="actions-col"><div class="action-icon-group"><button class="delete-btn" onclick="openDeleteModal('${file.name.replaceAll("'", "\\'")}', '${folderPath.replaceAll("'", "\\'")}', true)" title="删除文件夹">🗑️</button></div></td>`;
          fileTableContent += '</tr>';
        } else {
          ...
          fileTableContent += `<button class="move-btn" onclick="openMoveModal('${file.name.replaceAll("'", "\\'")}', '${filePath.replaceAll("'", "\\'")}' )" title="移动文件">📂</button>`;
          fileTableContent += `<button class="rename-btn" onclick="openRenameModal('${file.name.replaceAll("'", "\\'")}', '${filePath.replaceAll("'", "\\'")}' )" title="重命名">✏️</button>`;
          fileTableContent += `<button class="delete-btn" onclick="openDeleteModal('${file.name.replaceAll("'", "\\'")}', '${filePath.replaceAll("'", "\\'")}', false)" title="删除文件">🗑️</button>`;
  ...
  async function uploadNextFile() {
    if (currentIndex >= files.length) {
      ...
      if (failedFiles.length === 0) {
        ...
        progressText.textContent = `${files.length - failedFiles.length}/${files.length} 个已上传。失败：${failedList}`;
    ...
    progressText.textContent = `正在上传 ${file.name} (${currentIndex + 1}/${files.length})${methodText}`;

    const onProgress = (loaded, total) => {
      const percent = Math.round((loaded / total) * 100);
      progressFill.style.width = percent + '%';
      const speed = ''; // Could calculate speed here
      progressText.textContent = `正在上传 ${file.name} (${currentIndex + 1}/${files.length})${methodText} — ${percent}%`;
    };
