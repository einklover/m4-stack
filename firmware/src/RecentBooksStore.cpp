...
bool RecentBooksStore::saveToFile() const {
  ...
  for (const auto& book : recentBooks) {
    ...
    serialization::writeString(outputFile, book.originalSourcePath);
  }
...
bool RecentBooksStore::loadFromFile() {
  ...
  if (version != RECENT_BOOKS_FILE_VERSION) {
    ...
    for (uint8_t i = 0; i < count; i++) {
      std::string path, title, author, coverBmpPath, originalSourcePath;
      serialization::readString(inputFile, path);
      serialization::readString(inputFile, title);
      serialization::readString(inputFile, author);
      serialization::readString(inputFile, coverBmpPath);
      serialization::readString(inputFile, originalSourcePath);
      recentBooks.push_back({path, title, author, coverBmpPath, originalSourcePath});
    }
  }
