...
static const char* getChineseName(const char* englishName) {
    ...
    // --------------- 1. 主分类标题映射 ---------------
    if (strcmp(englishName, "Display") == 0) return "1)显示";
    if (strcmp(englishName, "Controls") == 0) return "2)按钮";
    if (strcmp(englishName, "System") == 0) return "3)系统";
    ...
    if (strcmp(englishName, "Set Custom Font Family") == 0) return "自定义阅读页面字体";
    ...
    if (strcmp(englishName, "Check for updates") == 0) return "当前版本：" CROSSPOINT_VERSION;
    if (strcmp(englishName, "Custom Status Bar Text") == 0) return "自定义状态栏文字";
    ...
    if (strcmp(englishName, "Full Refresh") == 0) return "全刷";
    if (strcmp(englishName, "Confirm") == 0) return "确认";
    // 休眠时间
    ...
    if (strcmp(englishName, "Up") == 0) return "上";
    if (strcmp(englishName, "Down") == 0) return "下";
