...
CrossPointPosition ProgressMapper::toCrossPoint(const std::shared_ptr<Epub>& epub, const KOReaderPosition& koPos,
  ...
  if (xpathSpineIndex >= 0 && xpathSpineIndex < epub->getSpineItemsCount()) {
    ...
    // Fall back to percentage-based lookup for spine index
  ...
  // Always estimate page number within the spine item using percentage.
  // This provides a more accurate position than always going to page 0,
  // and works correctly even when jumping to a different spine item
  // (the caller should use jumpToPercent/pendingSpineProgress for proper
  // positioning after section load, but this gives a reasonable display estimate).
