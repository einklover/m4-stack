// CffReader — streamed Compact Font Format 1/2 parser for OpenType CFF faces.
#pragma once
#include <cstddef>
#include <cstdint>
#include <vector>
...
class CffFont {
 public:
  ...
  bool isCff2() const{return cff2_;}
  ...
  // Only compressed format-3 ranges are normally resident. Uncompressed
  // format 0 and wide format 4 stay on SD and are queried by glyph id.
  uint32_t fdSelectResidentBytes() const{return fdSelectData_?fdSelectLen_:0;}
  ...
  void clearScratch();
 private:
  struct IndexInfo { Slice whole; uint32_t count=0; uint8_t offSize=0; uint32_t offsetsOff=0,dataOff=0; };
  struct Table { uint32_t off=0,len=0; bool present=false; };
  struct FdInfo { Slice privateDict; IndexInfo localSubrsInfo; Slice localSubrs; uint16_t defaultVsIndex=0; };
  ...
  bool parseIndex2(uint32_t relOff,IndexInfo& out,uint32_t* nextRel=nullptr) const;
  bool indexObject(const IndexInfo& index,uint32_t item,Slice& object) const;
  bool parseTopDict(Slice dict);
  bool parsePrivateDict(); // legacy non-CID helper retained for compatibility
  bool parsePrivateSubrs(Slice dict,IndexInfo& outInfo,Slice& outSlice,uint16_t* defaultVsIndex=nullptr);
  ...
  // CFF1 CID and CFF2 FontDICT metadata share this compact representation.
  // CharStrings/LocalSubrs and CFF2 VariationStore remain on SD.
  bool cidKeyed_=false; uint32_t fdArrayRel_=0,fdSelectRel_=0,variationStoreRel_=0;
  FdInfo* fdInfos_=nullptr; uint16_t fdCount_=0,fdCapacity_=0;
  mutable uint16_t activeDefaultVsIndex_=0;
  ...
  // CFF2 allows up to 513 operands. Allocate that stack lazily in PSRAM rather
  // than adding ~2KB to the task's internal stack on every glyph render.
  mutable float* operandScratch_=nullptr; mutable uint32_t operandScratchCap_=0;
