...
void CffFont::clearScratch(){
  ...
  freeMem(operandScratch_); operandScratch_=nullptr; operandScratchCap_=0;
...
bool CffFont::rasterize(uint16_t gid,uint16_t sizePx,GlyphBitmap&out){
  out={};if(!ready_||gid>=glyphCount_||!unitsPerEm_)return false;if(!prepareGlyphLocalSubrs(gid)){lastError_="failed to select CFF Font DICT";return false;}int32_t adv=0,lsb=0;if(!glyphHMetrics(gid,adv,lsb))return false;const float scale=float(sizePx)/float(unitsPerEm_);out.advance=int16_t(std::lround(adv*scale));
