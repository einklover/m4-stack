...
bool Xtc::scaleCoverToThumb(int width, int height) const {
  std::string coverPath = getCoverBmpPath();
  std::string thumbPath = getThumbBmpPath(width, height);
...
bool Xtc::generateThumbBmp(int width, int height) const {
  if (generateCoverBmp()) {
    return scaleCoverToThumb(width, height); // fallback to simple copy/scale if needed
  }
  ...
  if (scale >= 1.0f) {
    ...
    if (generateCoverBmp()) {
      ...
      if (SdMan.openFileForRead("XTC", getCoverBmpPath(), src)) {
        if (SdMan.openFileForWrite("XTC", getThumbBmpPath(width, height), dst)) {
          uint8_t buffer[512];
          while (src.available()) {
      ...
      return SdMan.exists(getThumbBmpPath(width, height).c_str());
    }
    return false;
  }
  ...
  Serial.printf("[%lu] [XTC] Generated thumb BMP (%dx%d) with Atkinson dithering: %s\n", millis(),
                thumbWidth, thumbHeight, getThumbBmpPath(width, height).c_str());
  return true;
}
