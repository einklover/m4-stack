...
uint32_t getChapterstartpage(int index) {
    if (index < 0 || index >= (int)m_chapters.size()) return 0;
    return m_chapters[index].startPage;
}

std::string getChapterTitleByIndex(int index) {
    if (index < 0 || index >= (int)m_chapters.size()) return "";
    return m_chapters[index].name;
}
