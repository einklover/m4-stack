...
void TextBlock::render(const GfxRenderer& renderer, const int fontId, const int x, const int y, const int underlineOffset) const {
  ...
  for (size_t i = 0; i < words.size(); i++) {
    ...
    if ((currentStyle & EpdFontFamily::UNDERLINE) != 0) {
      ...
      // y is the top of the text line; add ascender to reach baseline, then use configured underline offset (default 2px)
      const int underlineY = y + renderer.getFontAscenderSize(fontId) + underlineOffset;
