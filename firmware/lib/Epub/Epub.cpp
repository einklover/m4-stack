...
#include "../../src/util/M4IndexingState.h"

namespace {
uint32_t indexNowMs() { return millis(); }
void logIndex() {
  const auto line = M4IndexingState::formatLogLine(M4IndexingState::Session::get().snap());
  Serial.printf("[%lu] %s\n", millis(), line.c_str());
}
}  // namespace
...
bool Epub::load(const bool buildIfMissing, const bool skipLoadingCss) {
  ...
  if (bookMetadataCache->load()) {
    if (!skipLoadingCss && !loadCssRulesFromCache()) {
      ...
      parseCssFiles();  // writes css_rules.cache (even if empty) so we skip this path on next load
    }
  ...
  idx.reset();
  idx.beginPreparing("build_spine_toc");
  logIndex();
  M4IndexingState::ScopeGuard guard("index_aborted", indexNowMs);
  const uint32_t indexingStart = millis();
  uint32_t lastHb = indexingStart;

  auto failOut = [&](const char* msg) -> bool {
    bookMetadataCache->abortBuild(msg);
    idx.fail(millis() - indexingStart, msg);
    logIndex();
    guard.markSucceeded();  // already terminal via fail()
    return false;
  };

  auto heartbeat = [&](int pct, uint32_t items, const char* msg) {
    const uint32_t now = millis();
    const uint32_t elapsed = now - indexingStart;
    idx.reportProgress(pct, items, elapsed, msg, static_cast<uint32_t>(ESP.getFreeHeap()));
    if (elapsed - lastHb >= 2000 || pct >= 100) {
      lastHb = elapsed;
      logIndex();
    }
  };
  ...
  if (!bookMetadataCache->beginWrite()) {
    ...
    return failOut("begin_write");
  }
  idx.beginIndexing(0, "build_spine_toc");
  heartbeat(5, 0, "begin_write");

  const uint32_t opfStart = millis();
  BookMetadataCache::BookMetadata bookMetadata;
  if (!bookMetadataCache->beginContentOpfPass()) {
    Serial.printf("[%lu] [EBP] Could not begin writing content.opf pass\n", millis());
    return failOut("begin_opf");
  }
  if (!parseContentOpf(bookMetadata)) {
    Serial.printf("[%lu] [EBP] Could not parse content.opf\n", millis());
    return failOut("parse_opf");
  }
  heartbeat(35, static_cast<uint32_t>(bookMetadataCache->getSpineCount()), "opf");
  if (!bookMetadataCache->endContentOpfPass()) {
    Serial.printf("[%lu] [EBP] Could not end writing content.opf pass\n", millis());
    return failOut("end_opf");
  }
  ...
  if (!bookMetadataCache->endTocPass()) {
    ...
    return failOut("end_toc");
  }
  Serial.printf("[%lu] [EBP] TOC pass completed in %lu ms\n", millis(), millis() - tocStart);
  heartbeat(55, static_cast<uint32_t>(bookMetadataCache->getTocCount()), "toc");

  if (!bookMetadataCache->endWrite()) {
    Serial.printf("[%lu] [EBP] Could not end writing cache\n", millis());
    return failOut("end_write");
  }
  heartbeat(70, 0, "end_write");

  const uint32_t buildStart = millis();
  if (!bookMetadataCache->buildBookBin(filepath, bookMetadata)) {
    Serial.printf("[%lu] [EBP] Could not update mappings and sizes\n", millis());
    return failOut("build_book_bin");
  }
  ...
  idx.succeed(millis() - indexingStart, "ready");
  logIndex();
  guard.markSucceeded();
  Serial.printf("[%lu] [EBP] Loaded ePub: %s\n", millis(), filepath.c_str());
  return true;
...
bool Epub::generateThumbBmp(int width, int height) const {
  ...
  if (coverImageHref.empty()) {
    ...
    // Generate 1-bit BMP at exact display dimensions to avoid runtime scaling
    ...
    Serial.printf("[%lu] [EBP] Generated thumb BMP (%dx%d) from JPG cover image, success: %s\n", millis(),
                  width, height, success ? "yes" : "no");
    return success;
  } else {
  ...
  FsFile thumbBmp;
  SdMan.openFileForWrite("EBP", getThumbBmpPath(width, height), thumbBmp);
  thumbBmp.close();
  return false;
}
