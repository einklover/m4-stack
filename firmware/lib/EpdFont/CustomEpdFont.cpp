...
CustomEpdFont::CustomEpdFont(const String& filePath, const EpdFontData* data, uint32_t offsetIntervals,
  ...
  // 预打开 SD 卡字体文件
  SdMan.openFileForRead("CustomFont", filePath.c_str(), fontFile);
  if (fontFile.isOpen()) {
    Serial.printf("[Font] Pre-opened SD font file: %s\n", filePath.c_str());
  }
}

CustomEpdFont::~CustomEpdFont() {
  // 输出缓存统计
  const uint32_t totalGlyph = glyphCacheHits + glyphCacheMisses;
  const uint32_t totalBitmap = bitmapCacheHits + bitmapCacheMisses;
  if (totalGlyph > 0 || totalBitmap > 0) {
    Serial.printf("[Font] Cache stats - Glyph: %u/%u (%.1f%%), Bitmap: %u/%u (%.1f%%)\n",
                  glyphCacheHits, totalGlyph, totalGlyph > 0 ? (100.0 * glyphCacheHits / totalGlyph) : 0,
                  bitmapCacheHits, totalBitmap, totalBitmap > 0 ? (100.0 * bitmapCacheHits / totalBitmap) : 0);
  }
...
const EpdGlyph* CustomEpdFont::getGlyph(uint32_t cp, const EpdFontStyles::Style style) const {
  ...
  while (true) {
    ...
    if (foundInterval) {
      ...
      bool fileReady = fontFile.seekSet(glyphFileOffset);
      if (!fileReady) {
      ...
      if (version == 1) {
        // New format (16 bytes)
        uint8_t glyphBuf[16];
        size_t readBytes = fontFile.read(glyphBuf, 16);
        if (readBytes != 16) {
        ...
        */
        ...
        // Old format (13 bytes)
        uint8_t glyphBuf[13];
        size_t readBytes = fontFile.read(glyphBuf, 13);
        if (readBytes != 13) {
...
const uint8_t* CustomEpdFont::loadGlyphBitmap(const EpdGlyph* glyph, uint8_t* buffer,
  ...
  // 确保文件已打开并 seek 到正确位置
  uint32_t fileOffset = offsetBitmaps + offset;
  ...
  bool fileReady = fontFile.seekSet(fileOffset);
