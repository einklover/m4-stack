...
class FreeInkDisplay {
 public:
  ...
  // Waveform Lab: run one old->new frame transition with a caller-supplied
  // 110-byte LUT.  `lut` may be nullptr to keep whatever LUT is active.  The
  // panel is left powered (no auto power-down) so consecutive experiments can
  // be timed back-to-back.  Returns elapsed ms of the BUSY wait (0 if the
  // driver is not SSD1677-class).
  uint32_t waveformLabRefresh(const uint8_t* prev, const uint8_t* next, const uint8_t* lut,
                              bool turnOff = false);
  // Waveform Lab baseline: absolute FULL refresh to the given frame.
  void waveformLabBaseline(const uint8_t* frame);
  // Waveform Lab windowed diff refresh: only the (byte-aligned) window is
  // driven — BW=next window, RED=prev window, custom LUT.  Used for
  // strip-by-strip page-turn animation.  Returns BUSY wait ms.
  uint32_t waveformLabRefreshWindow(const uint8_t* prev, const uint8_t* next, const uint8_t* lut,
                                    uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                                    bool syncAfter = true);
  // Window refresh with caller-provided RED/BW window buffers (each w/8 x h
  // bytes, full height).  RED=bwWinRed, BW=bwWinBw, custom LUT.  Used by the
  // sliding-window animation so the window's RED matches the real on-panel
  // state (left=already-wiped new page, right=old page) — equivalent to a
  // cropped composeWipe frame, without processing the off-window region.
  uint32_t waveformLabRefreshWindowBufs(const uint8_t* redWin, const uint8_t* bwWin, const uint8_t* lut,
                                        uint16_t x, uint16_t y, uint16_t w, uint16_t h);
  // Dual-dose pipeline primitives (LUT session kept by caller via setCustomLUT
  // or the lut arg on activate):
  //   writeDiffWindow → activate → equalizeWindow (selective).
  void waveformLabWriteDiffWindow(const uint8_t* prev, const uint8_t* next, uint16_t x, uint16_t y, uint16_t w,
                                  uint16_t h);
  // Master activation on the current RAM window (last writeDiff / setRamArea).
  uint32_t waveformLabActivate(const uint8_t* lut = nullptr);
  // setRamArea(window) + Fast activate + restore full RAM window. Pixels outside
  // the rectangle are not scanned (status bar can be excluded from body wipes).
