...
// PSRAM-first window buffer: refresh windows are up to 47KB (full X3 panel);
// keeping them off internal RAM preserves the contiguous blocks mbedTLS needs
// for handshakes (the 40KB jsonGet gate). CPU-polled SPI (spiWriteNL) reads
// PSRAM fine — no DMA involved.
class WindowBuf {
 public:
  explicit WindowBuf(size_t n) {
    if (n == 0) return;
#if defined(ARDUINO_ARCH_ESP32)
    p_ = static_cast<uint8_t*>(heap_caps_malloc(n, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
#endif
    if (!p_) p_ = static_cast<uint8_t*>(malloc(n));
    n_ = n;
  }
  ~WindowBuf() { free(p_); }
  WindowBuf(const WindowBuf&) = delete;
  WindowBuf& operator=(const WindowBuf&) = delete;
...
void Ssd1677Driver::refresh(EpdBus& bus, RefreshMode mode, bool turnOff, bool async) {
...
#if defined(ARDUINO_ARCH_ESP32)
  Serial.printf("[SSD1677] refresh seq-path ctrl2=0x%02X mode=%d customLut=%d\n",
                static_cast<unsigned>(seqOverride), static_cast<int>(mode),
                _customLutActive ? 1 : 0);
#endif
  ...
  if (!async) bus.waitRefreshComplete("refresh");
#if defined(ARDUINO_ARCH_ESP32)
  Serial.printf("[SSD1677] refresh incr-path ctrl2=0x%02X mode=%d customLut=%d\n",
                static_cast<unsigned>(displayMode), static_cast<int>(mode),
                _customLutActive ? 1 : 0);
#endif
#if defined(SSD1677_PROBE_DEBUG) && SSD1677_PROBE_DEBUG
...
void Ssd1677Driver::displayWindow(EpdBus& bus, const uint8_t* fb, const uint8_t* prev, uint16_t x, uint16_t y,
  ...
  WindowBuf windowBuffer(windowBufferSize);
  ...
  for (uint16_t row = 0; row < h; row++) {
    ...
    memcpy(&windowBuffer.data()[dstOffset], &fb[srcOffset], windowWidthBytes);
  }
  ...
  if (prev != nullptr) {
    WindowBuf previousWindow(windowBufferSize);
    if (previousWindow) {
      for (uint16_t row = 0; row < h; row++) {
        const uint16_t srcY = y + row;
        const uint32_t srcOffset = static_cast<uint32_t>(srcY) * _wb + (x / 8);
        const uint32_t dstOffset = static_cast<uint32_t>(row) * windowWidthBytes;
        memcpy(&previousWindow.data()[dstOffset], &prev[srcOffset], windowWidthBytes);
      }
...
void Ssd1677Driver::waveformLabSyncWindow(EpdBus& bus, const uint8_t* next, uint16_t x, uint16_t y, uint16_t w,
                                          uint16_t h) {
  ...
  for (uint16_t row = 0; row < h; row++) {
    ...
    const uint32_t srcOffset = static_cast<uint32_t>(srcY) * _wb + (x / 8);
    const uint32_t dstOffset = static_cast<uint32_t>(row) * windowWidthBytes;
    memcpy(&windowBuffer[dstOffset], &next[srcOffset], windowWidthBytes);
  }
  // Hard equalize (no refresh): RED = BW = next inside the window.
  // Fast activation always diffs the whole panel; if a driven strip is left
  // with RED=old and BW=new, every later refresh re-drives it (continuous flash).
  // Writing both planes to the final content locks the strip quiet.
  setRamArea(bus, x, y, w, h);
  writeRam(bus, CMD_WRITE_RAM_BW, windowBuffer.data(), windowBufferSize);
  writeRam(bus, CMD_WRITE_RAM_RED, windowBuffer.data(), windowBufferSize);
}

void Ssd1677Driver::waveformLabWriteDiffWindow(EpdBus& bus, const uint8_t* prev, const uint8_t* next, uint16_t x,
                                               uint16_t y, uint16_t w, uint16_t h) {
  if (!prev || !next || w == 0 || h == 0) return;
  if (x + w > _w || y + h > _h) return;
  if (x % 8 != 0 || w % 8 != 0) return;
  const uint16_t windowWidthBytes = w / 8;
  const uint32_t windowBufferSize = static_cast<uint32_t>(windowWidthBytes) * h;
  std::vector<uint8_t> bwBuf(windowBufferSize);
  std::vector<uint8_t> redBuf(windowBufferSize);
  for (uint16_t row = 0; row < h; row++) {
    const uint16_t srcY = y + row;
    const uint32_t srcOffset = static_cast<uint32_t>(srcY) * _wb + (x / 8);
    const uint32_t dstOffset = static_cast<uint32_t>(row) * windowWidthBytes;
    memcpy(&bwBuf[dstOffset], &next[srcOffset], windowWidthBytes);
    memcpy(&redBuf[dstOffset], &prev[srcOffset], windowWidthBytes);
  }
  // Differential strip only — no master activation (caller activates once for
  // the whole active band pipeline).
  setRamArea(bus, x, y, w, h);
  writeRam(bus, CMD_WRITE_RAM_BW, bwBuf.data(), windowBufferSize);
  writeRam(bus, CMD_WRITE_RAM_RED, redBuf.data(), windowBufferSize);
}

void Ssd1677Driver::waveformLabActivate(EpdBus& bus) {
  // Drive RED≠BW inside the *current* RAM window (last setRamArea / writeDiff).
  // SSD1677 master activation (0x20) is window-scoped — columns outside the
  // active X/Y range are not scanned (off-panel). Callers that need a specific
  // rectangle should use waveformLabActivateWindow.
  refresh(bus, RefreshMode::Fast, /*turnOff=*/false);
}

void Ssd1677Driver::waveformLabActivateWindow(EpdBus& bus, uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  if (w == 0 || h == 0) return;
  if (x + w > _w || y + h > _h) return;
  if (x % 8 != 0 || w % 8 != 0) return;
  // Limit scan to this rectangle so status/body regions can refresh independently
  // without re-driving the rest of the panel (ghosting source when full-scan).
  setRamArea(bus, x, y, w, h);
  refresh(bus, RefreshMode::Fast, /*turnOff=*/false);
  // Restore full-panel window so subsequent full-frame writes/seeds are not
  // accidentally clipped to the last partial activate.
  setRamArea(bus, 0, 0, _w, _h);
}

void Ssd1677Driver::waveformLabRefreshWindowBufs(EpdBus& bus, const uint8_t* redWin, const uint8_t* bwWin,
                                                 uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  if (!redWin || !bwWin || w == 0 || h == 0) return;
  if (x + w > _w || y + h > _h) return;
  if (x % 8 != 0 || w % 8 != 0) return;
  const uint16_t wb = w / 8;
  const uint32_t winBytes = static_cast<uint32_t>(wb) * h;
  setRamArea(bus, x, y, w, h);
  writeRam(bus, CMD_WRITE_RAM_RED, redWin, winBytes);
  writeRam(bus, CMD_WRITE_RAM_BW, bwWin, winBytes);
  refresh(bus, RefreshMode::Fast, /*turnOff=*/false);
}

void Ssd1677Driver::seedPreviousFrame(EpdBus& bus, const uint8_t* buf) {  if (!buf) return;
...
#if defined(ARDUINO_ARCH_ESP32)
  Serial.printf("[SSD1677] setCustomLut active=true (105B + tail)\n");
#endif
}
