...
class PanelDriver {
 public:
  ...
  // Ensure the panel is powered (clock+analog on) so a differential refresh
  // runs the requested waveform instead of being promoted to an absolute
  // clean. No-op when already on. Default: nothing.
  virtual void powerOnIfNeeded(EpdBus& bus) { (void)bus; }
  // Waveform Lab: equalize window RED=BW=`next` (no refresh). Default: no-op.
  virtual void waveformLabSyncWindow(EpdBus& bus, const uint8_t* next, uint16_t x, uint16_t y, uint16_t w,
                                     uint16_t h) {
    (void)bus; (void)next; (void)x; (void)y; (void)w; (void)h;
  }
  // Waveform Lab: write RED=prev / BW=next for a window only (no refresh).
  virtual void waveformLabWriteDiffWindow(EpdBus& bus, const uint8_t* prev, const uint8_t* next, uint16_t x,
                                          uint16_t y, uint16_t w, uint16_t h) {
    (void)bus; (void)prev; (void)next; (void)x; (void)y; (void)w; (void)h;
  }
  // Waveform Lab: master activation only (current RAM window from last setRamArea;
  // custom LUT if already set). SSD1677 scans the active RAM window only.
  virtual void waveformLabActivate(EpdBus& bus) { (void)bus; }
  // setRamArea then activate; outside the rectangle is not scanned. Default: no-op.
  virtual void waveformLabActivateWindow(EpdBus& bus, uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
    (void)bus; (void)x; (void)y; (void)w; (void)h;
  }
  virtual void waveformLabRefreshWindowBufs(EpdBus& bus, const uint8_t* redWin, const uint8_t* bwWin,
                                            uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
    (void)bus; (void)redWin; (void)bwWin; (void)x; (void)y; (void)w; (void)h;
  }
