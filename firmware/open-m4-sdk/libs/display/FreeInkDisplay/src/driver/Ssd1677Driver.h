...
class Ssd1677Driver : public PanelDriver {
 public:
  ...
  void powerOnIfNeeded(EpdBus& bus) override { powerOn(bus); }
  ...
  // Waveform Lab: equalize window RED=BW=next (no refresh).
  void waveformLabSyncWindow(EpdBus& bus, const uint8_t* next, uint16_t x, uint16_t y, uint16_t w, uint16_t h);
  // Waveform Lab: write RED=prev / BW=next for a window only (no refresh).
  // Leaves controller RAM window (0x44/0x45) on this rectangle so a following
  // activate scans only this region (SSD1677 master activation is window-scoped).
  void waveformLabWriteDiffWindow(EpdBus& bus, const uint8_t* prev, const uint8_t* next, uint16_t x, uint16_t y,
                                  uint16_t w, uint16_t h);
  // Master activation only — scans whatever RAM window was last set by setRamArea
  // (e.g. after writeDiffWindow). Does not rewrite RAM.
  void waveformLabActivate(EpdBus& bus);
  // setRamArea(window) then master activation (Fast). Outside the window is not
  // scanned (treated as off-panel). Restores full-panel RAM window after BUSY.
  void waveformLabActivateWindow(EpdBus& bus, uint16_t x, uint16_t y, uint16_t w, uint16_t h);
  // Window refresh with ready-made RED/BW window buffers (w/8 x h bytes each).
  void waveformLabRefreshWindowBufs(EpdBus& bus, const uint8_t* redWin, const uint8_t* bwWin, uint16_t x,
