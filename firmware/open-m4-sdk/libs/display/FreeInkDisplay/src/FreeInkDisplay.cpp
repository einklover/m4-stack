...
uint32_t FreeInkDisplay::waveformLabRefresh(const uint8_t* prev, const uint8_t* next, const uint8_t* lut,
                                            bool turnOff) {
  if (!_driver) return 0;
  syncPendingAsync();
  _shadowValid = false;
  _redRamSynced = false;
  // A differential FAST must not be promoted to an absolute clean: after a
  // FULL baseline the panel may be powered down, which displayImpl would
  // "fix" by promoting Fast -> Half/Full (losing the custom LUT entirely).
  _driver->powerOnIfNeeded(_bus);
  if (lut != nullptr) _driver->setCustomLut(_bus, true, lut);
  const uint32_t t0 = millis();
  // Fast diff refresh: BW = next, RED = prev.  Blocking (BUSY waited inside).
  _driver->display(_bus, next, prev, freeink::RefreshMode::Fast, turnOff);
  const uint32_t elapsed = millis() - t0;
#if defined(ARDUINO_ARCH_ESP32)
  Serial.printf("[LAB] waveformLabRefresh elapsed=%ums lut=%s\n", static_cast<unsigned>(elapsed),
                lut ? "custom" : "none");
#endif
  if (lut != nullptr) _driver->setCustomLut(_bus, false, nullptr);
  return elapsed;
}

uint32_t FreeInkDisplay::waveformLabRefreshWindow(const uint8_t* prev, const uint8_t* next, const uint8_t* lut,
                                                  uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                                                  bool syncAfter) {
  if (!_driver || !prev || !next) return 0;
  syncPendingAsync();
  _shadowValid = false;
  _redRamSynced = false;
  _driver->powerOnIfNeeded(_bus);
  if (lut != nullptr) _driver->setCustomLut(_bus, true, lut);
  const uint32_t t0 = millis();
  // Windowed diff: BW window = next, RED window = prev.  The driver window
  // path rewrites both planes inside the window and refreshes (Fast).
  // Note: master activation still diffs the *full* panel RAM — only columns
  // with RED≠BW move.  Keep already-scanned strips equalized (syncAfter).
  _driver->displayWindow(_bus, next, prev, x, y, w, h, /*turnOff=*/false);
  // syncAfter=true: hard equalize RED=BW=next in this window (no refresh) so
  // later full-panel activations do not re-drive this strip.
  // syncAfter=false: leave RED=prev for another multipass pulse on the same strip.
  if (syncAfter) {
    _driver->waveformLabSyncWindow(_bus, next, x, y, w, h);
  }
  const uint32_t elapsed = millis() - t0;
#if defined(ARDUINO_ARCH_ESP32)
  Serial.printf("[LAB] window %u,%u %ux%u elapsed=%ums\n", static_cast<unsigned>(x),
                static_cast<unsigned>(y), static_cast<unsigned>(w), static_cast<unsigned>(h),
                static_cast<unsigned>(elapsed));
#endif
  if (lut != nullptr) _driver->setCustomLut(_bus, false, nullptr);
  return elapsed;
...
uint32_t FreeInkDisplay::waveformLabRefreshWindowBufs(const uint8_t* redWin, const uint8_t* bwWin,
                                                      const uint8_t* lut, uint16_t x, uint16_t y, uint16_t w,
                                                      uint16_t h) {
  if (!_driver || !redWin || !bwWin) return 0;
  syncPendingAsync();
  _shadowValid = false;
  _redRamSynced = false;
  _driver->powerOnIfNeeded(_bus);
  if (lut != nullptr) _driver->setCustomLut(_bus, true, lut);
  const uint32_t t0 = millis();
  _driver->waveformLabRefreshWindowBufs(_bus, redWin, bwWin, x, y, w, h);
  const uint32_t elapsed = millis() - t0;
  if (lut != nullptr) _driver->setCustomLut(_bus, false, nullptr);
  return elapsed;
}

void FreeInkDisplay::waveformLabWriteDiffWindow(const uint8_t* prev, const uint8_t* next, uint16_t x, uint16_t y,
                                                uint16_t w, uint16_t h) {
  if (!_driver || !prev || !next) return;
  syncPendingAsync();
  _shadowValid = false;
  _redRamSynced = false;
  _driver->powerOnIfNeeded(_bus);
  _driver->waveformLabWriteDiffWindow(_bus, prev, next, x, y, w, h);
}

uint32_t FreeInkDisplay::waveformLabActivate(const uint8_t* lut) {
  if (!_driver) return 0;
  syncPendingAsync();
  _shadowValid = false;
  _redRamSynced = false;
  ...
  if (lut != nullptr) _driver->setCustomLut(_bus, true, lut);
  const uint32_t t0 = millis();
  _driver->waveformLabActivate(_bus);
  const uint32_t elapsed = millis() - t0;
  // Leave custom LUT active for the rest of a dual-dose session; caller clears
  // with setCustomLUT(false) when the animation ends.
  return elapsed;
}

uint32_t FreeInkDisplay::waveformLabActivateWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                                                   const uint8_t* lut) {
  if (!_driver || w == 0 || h == 0) return 0;
  syncPendingAsync();
  _shadowValid = false;
  _redRamSynced = false;
  _driver->powerOnIfNeeded(_bus);
  if (lut != nullptr) _driver->setCustomLut(_bus, true, lut);
  const uint32_t t0 = millis();
  _driver->waveformLabActivateWindow(_bus, x, y, w, h);
  const uint32_t elapsed = millis() - t0;
  // Leave custom LUT active for multi-step wipe sessions (caller clears).
  return elapsed;
}

void FreeInkDisplay::waveformLabEqualizeWindow(const uint8_t* frame, uint16_t x, uint16_t y, uint16_t w,
                                               uint16_t h) {
  if (!_driver || !frame) return;
  syncPendingAsync();
  _driver->powerOnIfNeeded(_bus);
  _driver->waveformLabSyncWindow(_bus, frame, x, y, w, h);
}

void FreeInkDisplay::waveformLabBaseline(const uint8_t* frame) {
  if (!_driver || !frame) return;
  syncPendingAsync();
  _shadowValid = false;
  _redRamSynced = false;
