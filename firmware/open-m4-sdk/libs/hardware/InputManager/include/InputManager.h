...
public:
  ...
  // True if this board has a touch controller configured (I2C bus brought up).
  // Does NOT mean the controller has completed settle / stream probe.
  bool hasTouch() const;
  // True only after settle deadline and a successful FT6x36 (or peer) probe/frame.
  bool isTouchStreamReady() const;
  // Raw panel ranges for diagnostics (0 if touch not configured).
  void getTouchRawRange(uint16_t& minX, uint16_t& maxX, uint16_t& minY, uint16_t& maxY) const;
  bool touchSwapXY() const;
  bool touchFlipX() const;
  bool touchFlipY() const;
...
private:
  ...
  // BoardConfig::ACTIVE.touch.controller (CHSC6x, FT6x36, or GT911).
  void beginTouch();
  ...
  void beginFt6x36();
  void pollFt6x36(unsigned long now);
  bool readFt6x36Frame(uint8_t *data, size_t len);
  void handleFt6x36SampleFailure(unsigned long now);
  ...
  bool touchDataEnabled = false; // I2C up, controller configured
  bool touchStreamReady = false; // settle + successful probe/frame
  ...
  unsigned long touchReadyAt = 0; // controller-specific startup settle deadline
  unsigned long touchReleaseAt = 0;
  uint8_t ft6x36FailedReads = 0;
  unsigned long ft6x36LastFailureLogAt = 0;
  bool ft6x36StreamLogged = false;
  ...
  static constexpr uint32_t FT6X36_I2C_HZ = 50000;
  static constexpr unsigned long FT6X36_SETTLE_MS = 3000;
  static constexpr unsigned long FT6X36_SAMPLE_DELAY_MS = 10;
  static constexpr uint8_t FT6X36_RELEASE_AFTER_FAILURES = 5;
  static constexpr uint8_t FT6X36_RECOVER_AFTER_FAILURES = 25;
