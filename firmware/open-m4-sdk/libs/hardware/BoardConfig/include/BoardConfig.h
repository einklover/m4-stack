...
// --- 2) Coherence: exactly one MCU family, at least one device ---------------
      ...
      FREEINK_DEVICE_MURPHY_M4 || FREEINK_DEVICE_DELINK || FREEINK_DEVICE_LILYGO || FREEINK_DEVICE_M5PAPER ||       \
      FREEINK_DEVICE_STICKY)
#error \
    "FreeInk: no device selected. Pass at least one -DFREEINK_DEVICE_<NAME> (X4, X3, X4PRO, M5, MURPHY, MURPHY_M4, DELINK, LILYGO, M5PAPER, STICKY) in your build env — see platformio.sample.ini."
#endif
...
#define FREEINK_MCU_S3                                                                                    \
  (FREEINK_DEVICE_M5 || FREEINK_DEVICE_MURPHY || FREEINK_DEVICE_MURPHY_M4 || FREEINK_DEVICE_DELINK ||      \
...
#if FREEINK_DEVICE_X4 || FREEINK_DEVICE_MURPHY_M4 || FREEINK_DEVICE_DELINK || FREEINK_DEVICE_STICKY || \
...
#define FREEINK_CAP_TOUCH                                                                         \
  (FREEINK_DEVICE_MURPHY || FREEINK_DEVICE_MURPHY_M4 || FREEINK_DEVICE_LILYGO || FREEINK_DEVICE_M5PAPER || \
   FREEINK_DEVICE_STICKY || FREEINK_DEVICE_X4PRO)
#endif
#ifndef FREEINK_CAP_FRONTLIGHT
#define FREEINK_CAP_FRONTLIGHT \
  (FREEINK_DEVICE_DELINK || FREEINK_DEVICE_MURPHY || FREEINK_DEVICE_MURPHY_M4 || FREEINK_DEVICE_LILYGO || \
   FREEINK_DEVICE_X4PRO)
#endif
...
#define FREEINK_CAP_BUZZER (FREEINK_DEVICE_STICKY || FREEINK_DEVICE_MURPHY || FREEINK_DEVICE_MURPHY_M4)
#endif
...
#define FREEINK_SD_SDMMC (FREEINK_DEVICE_MURPHY_M4 || FREEINK_DEVICE_DELINK || FREEINK_DEVICE_X4PRO)
#endif
...
enum class TouchController : uint8_t { None, Chsc6x, Ft6x36, Gt911 };
...
// Murphy M4 has no verified PCM codec path. GPIO46 is the independently tested
// LEDC buzzer output; keeping output=None prevents the audio UI from advertising
// unsupported WAV playback while still enabling the Buzzer capability.
constexpr AudioConfig MURPHY_M4_AUDIO = {
    AudioOutput::None, PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED,
    true,              PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, 0,              46};
...
// --- Murphy M4 (4.26") — ESP32-S3R8, SSD1677, FT6336U, SDMMC ----------------
//
// Pinout and controller protocol were recovered from SCH_ESP..18.pdf and then
// independently exercised on a production unit with the small CrossLua hardware
// probe. Unlike Murphy M3, M4 uses the 800x480 GDEQ0426T82-FT01C/SSD1677 panel,
// a 4-bit SDMMC slot, dual-channel frontlight, and an FT6x36-compatible touch
// controller at 0x2e.
constexpr BoardProfile MURPHY_M4 = {
    ...
    800,
    480,
    // EPD: SCLK4 MOSI3 CS5 DC6 RST7 BUSY8. The live probe was stable at
    // 10 MHz; keep that conservative clock until long-session waveform tests.
    {4, 3, 5, 6, 7, 8, PIN_UNASSIGNED},
    10000000,
    // SD is native 4-bit SDMMC (pins below). Only the active-low GPIO10 power
    // gate is meaningful in this SPI-shaped compatibility field.
    {PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, 10, false, 0, false},
    // Two side keys drive reader Up/Down; GPIO0 is the active-low lock/power key.
    // Back/confirm/directional UI actions come from coordinate touch.
    {PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, 1, 2, 0, false},
    9,               // battery ADC through a 2:1 divider
    PIN_UNASSIGNED,  // GPIO43 charge polarity is not yet safe to consume
    2.0f,
    PIN_UNASSIGNED,  // no independently verified USB-present GPIO
    // FT6336U-compatible frame at 0x2e: SDA13/SCL12, INT44, active-low
    // touch-power gate GPIO45. The sensor reports portrait X=0..479,Y=0..799;
    // swapXY + flipY aligns it to the panel-native 800x480 frame. In default
    // portrait UI this maps a raw touch (x,y) back to the same logical (x,y).
    {TouchController::Ft6x36, 13, 12, 44, PIN_UNASSIGNED, 0x2e, 0, 799, 0, 479, false, 0, true, false, 45,
     true, false, true, false, false},
    // Primary is cool GPIO48, secondary is warm GPIO47 (5 kHz, 8-bit PWM).
    {48, 5000, 8, true, 47},
    MURPHY_M4_AUDIO,
    NO_LEDS,
    NO_FLIP,
    // CLK16 CMD15 D0=17 D1=18 D2=11 D3=14, four-bit bus.
    {16, 15, 17, 18, 11, 14, 4},
    NO_GAUGE,
    NO_MIC,
    NO_SENSORS,
...
constexpr uint32_t MAX_FRAMEBUFFER_BYTES = cmax(
    ...
    cmax(cmax(cmax(FREEINK_DEVICE_MURPHY_M4 ? panelBytes(MURPHY_M4) : 0u,
                   FREEINK_DEVICE_DELINK ? panelBytes(DE_LINK) : 0u),
              cmax(FREEINK_DEVICE_LILYGO ? panelBytes(LILYGO_T5S3) : 0u,
