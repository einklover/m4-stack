...
// Structured last error for SDMMC bring-up (mapped by SDCardManager → M4SdStatus).
enum class SdmmcFailStage : uint8_t {
  None = 0,
  HostInit,
  SlotInit,
  CardInit,
...
enum class SdmmcFailCode : uint8_t {
  Ok = 0,
  HostInitFailed,
  SlotInitFailed,
  NoCard,
...
struct SdmmcLastError {
  SdmmcFailCode code = SdmmcFailCode::Ok;
  SdmmcFailStage stage = SdmmcFailStage::None;
  int attempt = 0;
  int espErr = 0;
  char detail[48] = {};
};
...
class SdmmcBlockDevice : public FsBlockDeviceInterface {
 public:
  ...
  const SdmmcLastError& lastError() const { return _lastError; }
 ...
 private:
  void setErr(SdmmcFailCode code, SdmmcFailStage stage, int attempt, int espErr, const char* detail);
