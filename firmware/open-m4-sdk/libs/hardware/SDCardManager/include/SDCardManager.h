...
class SDCardManager {
 public:
  ...
  // Last begin()/probe outcome for diagnostics (never formats media).
  // stage/code names align with M4SdStatus (host-testable policy header).
  const char* lastStage() const { return _lastStage; }
  const char* lastDetail() const { return _lastDetail; }
  const char* lastCodeName() const { return _lastCodeName; }
  int lastMountedPart() const { return _lastMountedPart; }
  uint8_t lastFatType() const { return _lastFatType; }
  uint64_t lastSectorCount() const { return _lastSectorCount; }

  // Read-only post-mount probe: list root; optionally open/read/seek a path if it exists.
  // Does not create files. Returns false on I/O failure.
  bool capabilityProbe(const char* optionalExistingPath = nullptr);
 ...
 private:
  ...
  const char* _lastStage = "idle";
  const char* _lastCodeName = "not_initialized";
  char _lastDetail[96] = {};
  int _lastMountedPart = -1;
  uint8_t _lastFatType = 0;
  uint64_t _lastSectorCount = 0;

  void setLast(const char* stage, const char* detail, const char* codeName = nullptr) {
    _lastStage = stage ? stage : "unknown";
    if (codeName) _lastCodeName = codeName;
    if (detail) {
      size_t i = 0;
      for (; detail[i] && i + 1 < sizeof(_lastDetail); ++i) _lastDetail[i] = detail[i];
      _lastDetail[i] = 0;
    } else {
      _lastDetail[0] = 0;
    }
