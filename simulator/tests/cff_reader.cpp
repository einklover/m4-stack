...
std::vector<uint8_t> makeCidCff(bool format3){
  ...
  std::vector<uint8_t> top;
  dictInt(top,0);dictInt(top,1);dictInt(top,0);top.push_back(12);top.push_back(30);
  const size_t csPatch=top.size();dictInt(top,0);top.push_back(17);
  ...
  addIndex(c,{fd0,fd1});
  const size_t fdDataStart=size_t(fdArrayRel)+9;

  const uint32_t fdSelectRel=uint32_t(c.size());
  if(format3){
    c.push_back(3);be16(c,2);
    be16(c,0);c.push_back(0);be16(c,2);c.push_back(1);be16(c,3);
  }else{
    c.push_back(0);c.push_back(0);c.push_back(0);c.push_back(1);
  }
  ...
  c.push_back(t2(2));c.push_back(19);addIndex(c,{arithmeticSquareSubr()});
