...
#include "CffReader.h"
namespace {
void be16(std::vector<uint8_t>&v,uint16_t x){v.push_back(x>>8);v.push_back(x);} void be32(std::vector<uint8_t>&v,uint32_t x){v.push_back(x>>24);v.push_back(x>>16);v.push_back(x>>8);v.push_back(x);} void put16(std::vector<uint8_t>&v,size_t o,uint16_t x){v[o]=x>>8;v[o+1]=x;} void put32(std::vector<uint8_t>&v,size_t o,uint32_t x){v[o]=x>>24;v[o+1]=x>>16;v[o+2]=x>>8;v[o+3]=x;} void dictInt(std::vector<uint8_t>&o,int32_t n){o.push_back(29);be32(o,uint32_t(n));} uint8_t t2(int n){return uint8_t(n+139);} void addIndex(std::vector<uint8_t>&c,const std::vector<std::vector<uint8_t>>&os){be16(c,uint16_t(os.size()));if(os.empty())return;c.push_back(2);uint16_t off=1;be16(c,off);for(auto&o:os){off+=o.size();be16(c,off);}for(auto&o:os)c.insert(c.end(),o.begin(),o.end());}
std::vector<uint8_t> makeCff(){std::vector<uint8_t>c={1,0,4,2};addIndex(c,{{'M','4'}});std::vector<uint8_t>top;dictInt(top,0);top.push_back(17);dictInt(top,2);dictInt(top,0);top.push_back(18);size_t ti=c.size();addIndex(c,{top});addIndex(c,{});addIndex(c,{});uint32_t cs=c.size();addIndex(c,{{14},{t2(0),t2(0),21,t2(-107),10,14}});uint32_t pr=c.size();c.push_back(t2(2));c.push_back(19);addIndex(c,{{t2(100),t2(0),t2(0),t2(100),t2(-100),t2(0),t2(0),t2(-100),5,11}});size_t d=ti+7;put32(c,d+1,cs);put32(c,d+12,pr);return c;}
void addTable(std::vector<uint8_t>&f,size_t rec,const char*t,const std::vector<uint8_t>&data){while(f.size()&3)f.push_back(0);uint32_t o=f.size();std::memcpy(f.data()+rec,t,4);put32(f,rec+8,o);put32(f,rec+12,data.size());f.insert(f.end(),data.begin(),data.end());}
