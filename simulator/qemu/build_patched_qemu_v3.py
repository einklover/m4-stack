#!/usr/bin/env python3
"""Build pinned Espressif QEMU using the cumulative Murphy v3 patch series."""
...
def load_series() -> list[Path]:
    ...
    if len(result) != len(set(result)):
        raise RuntimeError("duplicate patch entry in series-v3")
    return result
...
def prepare_source(src: Path, *, force: bool) -> None:
    ...
    if not subprocess.check_output(["git", "status", "--porcelain"], cwd=src, text=True).strip():
        raise RuntimeError("v3 patch series produced no source changes")
...
def configure_and_build(src: Path, *, jobs: int, reconfigure: bool) -> Path:
    build = src / "build-murphy-v3"
    binary = build / "qemu-system-xtensa"
    if reconfigure and build.exists():
        shutil.rmtree(build)
    if not build.exists():
        build.mkdir(parents=True)
        run([str(src / "configure"), "--target-list=xtensa-softmmu", "--disable-werror", "--disable-docs"], cwd=build)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--source-dir", default=str(Path.home() / ".cache/murphy-m4/espressif-qemu-v3"))
