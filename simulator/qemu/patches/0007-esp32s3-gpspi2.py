...
GPSPI_SOURCE = r'''/* ESP32-S3 general-purpose SPI (GPSPI2/3) functional model.
 *
 * This is intentionally separate from esp32s3_spi.c, which models the SPI1
 * memory/PSRAM path.  The S3 GP-SPI register layout is different from both the
 * old ESP32 GP-SPI block and SPI_MEM.  The first implementation covers the CPU
 * data-buffer transaction path used by firmware-visible SPI master operations.
 */
...
{
    qemu_set_irq(s->irq, (s->regs[R_DMA_INT_RAW >> 2] &
                          s->regs[R_DMA_INT_ENA >> 2]) != 0);
}
...
static void gpspi_transaction(ESP32S3GpSpiState *s)
{
    ...
    if (user & USER_COMMAND) {
        const unsigned bits = ((user2 >> 28) & 0xf) + 1;
        gpspi_send_msb(s, user2 & 0xffff, bits);
    }
    if (user & USER_ADDR) {
        const unsigned bits = ((user1 >> 27) & 0x1f) + 1;
        gpspi_send_msb(s, s->regs[R_ADDR >> 2], bits);
    }
    if (user & USER_DUMMY) {
        const unsigned cycles = (user1 & 0xff) + 1;
    ...
    if (user & (USER_MOSI | USER_MISO)) {
        ...
        unsigned bytes = (bits + 7) / 8;
        if (bytes > GPSPI_FIFO_WORDS * 4) {
            bytes = GPSPI_FIFO_WORDS * 4;
        }
        for (unsigned i = 0; i < bytes; ++i) {
...
static void gpspi_write(void *opaque, hwaddr addr, uint64_t value, unsigned size)
{
    ...
    switch (addr) {
    case R_CMD:
        /* UPDATE synchronizes APB-domain config into the peripheral. Functional
         * QEMU applies register writes synchronously, so UPDATE completes now.
         */
    ...
    case R_DMA_INT_SET:
        ...
        break;
    case R_DMA_INT_RAW:
        /* Generated S3 register describes RAW as software-clearable/settable;
         * keep direct writes useful for the low-level driver tests. */
        s->regs[R_DMA_INT_RAW >> 2] = v;
        gpspi_update_irq(s);
        break;
    default:
...
static void gpspi_init(Object *obj)
{
    ...
    memory_region_init_io(&s->iomem, obj, &gpspi_ops, s,
                          TYPE_ESP32S3_GPSPI, GPSPI_MMIO_SIZE);
...
def main(argv: list[str]) -> int:
    ...
    mtext = replace_once(
        ...
        "compile ESP32-S3 GP-SPI source",
    )
    ...
    text = replace_once(
        ...
        '#define TYPE_MURPHY_FT6X36 "murphy-ft6x36"\n'
        '#define TYPE_ESP32S3_GPSPI "ssi.esp32s3.gpspi"\n',
        "define S3 GP-SPI device type",
    )
    text = replace_once(
        text,
        "    SsiPsramState *psram;\n\n    uint32_t sens_regs",
        "    SsiPsramState *psram;\n    DeviceState *gpspi2;\n\n    uint32_t sens_regs",
        "retain GP-SPI2 device",
    )
    text = replace_once(
        text,
        """        for (int i = 0; i < 2; ++i) {
     ...
     */"""
    gpspi_realize = """    /* General-purpose SPI2. This is the user SPI host used by the
     * Murphy display stack, distinct from SPI1/MSPI flash/PSRAM.
     */
    sysbus_realize(SYS_BUS_DEVICE(ss->gpspi2), &error_fatal);
    memory_region_add_subregion_overlap(
        sys_mem, DR_REG_SPI2_BASE,
...
""" + anchor
    text = replace_once(text, anchor, gpspi_realize, "realize GP-SPI2 at S3 base")
