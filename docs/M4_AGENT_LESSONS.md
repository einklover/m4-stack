...
## 2026-09-01 — Round 17: dual-size Home covers must keep source.img

- Symptom: Home hero `cover_110x180.bmp` (110×180) ≠ mini `cover_74x106.bmp` (74×106); after hero async-load, opening another book moves the hero to the 3 mini slots but it stays wireframe. Root cause class: hero acquire only wrote the hero BMP, and `M4ProviderCoverCache::acquire` deleted a complete `source.img` on cancel/convert failure, so `ensureSizedCoverFromSource` for the mini could not rebuild 74×106 without Wi-Fi.
- Fix: (1) Whenever `ensureSizedCoverFromSource` or `acquire` produces a Home scene size (110×180 or 74×106) from `source.img` (or last-resort `cover_171x254.bmp`), also generate the complementary Home size from the **same** source in the same pass — one HTTP download → two 1-bit exact-size BMPs via the same `backend.convert(..., oneBit=true)` path. `acquireProviderCover`'s production `backend.convert` now branches `isHomeSceneSize ? oneBit:true : false` so Home hero/mini are 1-bit. (2) Download policy: if `source.img` already existed **or** the fetch returned ok, never delete `source.img` on later `cancel` or `convert` failure; only the failed target BMP is removed. Partial download (fetch !ok) still removes the partial `source.img`.
- Home refresh: `HomeActivity::refreshMissingCoversInCtx` stays as-is; because `tryEnsureCoverThumbInCtx` and `acquire` now dual-write, a hero that later becomes a mini is already a local hit (`decode cover_74x106.bmp` or `ensure` from existing `source.img` without Wi-Fi).
- Pitfall: `test_cover_last_resort.cpp` checks `pos171 > posSourceExists` via `find("cover_171x254")`. The helper `fallbackBmpPathInDir` returning `"/cover_171x254.bmp"` and early doc comments before the `if (backend.exists(source))` branch would make that check fail. Keep any early `cover_171x254` literal split (e.g. `"/cover_" "171x254.bmp"` or `cover_171 x254` in comments) so the first contiguous occurrence is the `Last resort: source missing -> try 2-bit Fengyan cover_171x254.bmp` comment **after** the source branch. The production fallback path is still `fallbackBmpPathInDir` (runtime concatenates correctly).
- Check: host pins `acquire` for 110 also asserts `mini` exists and both are `isValid1BitBmpOfSize`; `acquire` where `convert` fails asserts `source.img` still present; `ensureSized` for 110 asserts both exist and second ensure for 74 is `cacheHit` with 0 fetches.

## 2026-09-01 — Round 16: true async needs a paint gap; Home may acquire cover for never-opened detail (policy A)

- Symptom: R15 `publishHomeSceneWithAssetsFastCtx` then same-task `refreshMissingCoversInCtx` had no FreeRTOS yield — display never painted the placeholder frame; plus `ensureSizedCoverFromSource` alone never HTTP-fetches, so never-opened detail (no `source.img`) stayed wireframe forever.
- Root cause: "async" was only a comment — one-shot `backendLoop` without `vTaskDelay` or separate job has no paint gap; and Home never resolved `coverUrl` nor called `acquireProviderCover` for history URIs, while `RecentBook` stores `coverBmpPath` but not `coverUrl`.
- Fix: (1) after fast publish (`tryDecodeCoverThumbIfExists` only, no `ensureSized`/`acquire`) do `vTaskDelay(80ms)` then `refreshMissingCoversInCtx` — check `cancelled/epoch` after the yield so `onExit` aborts; second `publish()` upgrades placeholders → real covers. (2) In refresh, when local `ensureSized` still misses, parse `m4cp://pid/bid`, resolve `coverUrl` via `HomeCoverPolicyA::resolveCoverUrlFromShelf` (reads `/apps_data/<appId>/provider/shelf_rows.tsv`, field 4 for fanqie/jjwxc/weread, field 5 via `M4LegadoBridge::coverProxyUrl` for legado) then fallback `M4NativeProviderBookDetail::fetch` (bounded 48 KiB, Wi-Fi-gated for network providers), gate `acquireProviderCover` on `M4QemuNet::staConnected()` + `cancelled`, cap hero+3. Do not persist `coverUrl` in `RecentBooksStore` — read from existing caches.
- Contract: fast slice contains `tryDecodeCoverThumbIfExists` but not `ensureSized`/`acquire`; between fast and refresh there is `vTaskDelay`; refresh contains `acquireProviderCover` + `homeWifiConnected` + `parseHistoryUri` + `isCancelled`. Host test `test_home_cover_async_acquire.cpp` pins this.
- Pitfall: adding `M4QemuNet.h`/`M4NativeProviderBookDetail.h`/`M4LegadoBridge.h` only under `CROSSPOINT_MURPHY_M4` keeps host `g++-14` green; shelf scan must handle FatFS NUL slack (`line.empty() && c<0x20` skip) and cover field index per provider.
## 2026-09-01 — Round 18: hero 1-bit dither must not hide behind 8-bit success; brand binding needs live publish

- Symptom: after R17, returning to Home left hero `110×180` as wireframe. R17 forced `acquireProviderCover` for Home sizes through `JpegToBmpConverter::jpegFileTo1BitBmpStreamWithSize` / `bmpFileTo1BitBmpWithSize` (needs `heapHealthy(0x420)` + two `W*H` dither buffers). On fragmented Home heap that 1-bit path can fail where the old R16 `false` (8-bit) path succeeded, leaving `source.img` kept but no thumb. Fast path `exists(thumb)` then `decode` failed → placeholder, but `refreshMissingCoversInCtx` tested only `!exists(thumb)` so never retried, and no brand status was shown.
- Fix: (1) `M4ProviderCoverCache::acquire` for `isHomeSceneSize` now delegates to `ensureHomeSceneSizesFromSource(dir, backend, cancelled)` — same 1-bit exact path but through the ensure helper that dual-writes both Home sizes from the same `source.img` (or fallback `cover_171x254.bmp`), never fetches, never deletes complete `source.img`. Non-Home sizes keep 8-bit. (2) `refreshMissingCoversInCtx` now checks `hasAsset` (`draftPub.entries` contains hero/mini key) as well as `exists`; if thumb exists but asset missing → `SdMan.remove(thumb)` and regenerate. After each `tryEnsure` or `acquire`, if thumb still missing retry `tryEnsure` from the now-persisted `source.img`. (3) Brand: `HomeSceneModel` adds `kBindingBrandText=3` (`$home.brand_text`), `brandText` ref, `setBrandText()` (append+publish), default "Murphy M4" in `initialSnapshot`/`begin`/`resolve`. Theme widened `160→280` at `[24,24,280,24]` to fit ≤10 Han Chinese status without hitting battery at `x=432`. `compile_home_theme.py` adds `"$home.brand_text":3` to `BINDING_SCENE_MAP`; manifest `bindings: {"$home.brand_text":3}`. Status strings one at a time, published with `updateRequired` **before** slow work: `解析封面` (entry), `生成大封面`/`生成小封面` (ensure), `等待 Wi-Fi` (no wifi), `下载原图` (before fetch), `解码封面` (before decode), restored to `Murphy M4` on every cancel/complete/exit. `publishHomeSceneFromBackendCtx` also publishes `解析封面` before the 80 ms `vTaskDelay` gap so the first brand status is visible, not only after all dithers.
...
## 2026-09-22 — M4 app working memory belongs in PSRAM, Home is the release boundary
...
## 2026-09-24 — Reader task must feed TWDT while idle and during font reload

- Symptom/evidence: a real M4 rebooted shortly after entering Reader / opening its menu. Serial reported `task_wdt` for `TxtReaderActivi` with both CPU cores idle; the device reset reason was `ESP_RST_TASK_WDT` (6). `displayTaskLoop()` registered itself with TWDT but only fed during page layout, then delayed without feeding while idle or suppressed.
- Fix/check: feed TWDT once per display-task loop, including idle/menu waits. Font family/size reload also releases runtime TTF faces, so `openMenu()` must not proceed when its 400 ms render-lock attempt fails, and the display task must recheck suppression under lock before layout/index work. Exercise both contracts in `test_reader_ttf_quiescence_contract.py`. On the tested APP1 image, Reader stayed open past the TWDT window, changed 38→40 px, rendered HanYi and Noto Sans SC TTF, paged, and returned Home/reopened. A 20-cycle real-device size/reflow/page-turn run including Home/reopen passed; min free heap was 99,788→97,268 bytes, largest internal block 59,380→61,428 bytes, free PSRAM stayed 1,707,024 bytes, and reset reason stayed USB (11).

## 2026-09-26 — Audit SD recursion against the pinned SdFat implementation
...
## 2026-09-26 — FreeRTOS self-delete skips C++ unwinding; directory resume is a 32-byte slot

- `vTaskDelete` / `M4Psram::deleteTask(nullptr)` does not run destructors or code after an early `return` inside the task function. Provider and store workers must finish the business function, destroy locals, publish a terminal snapshot, clear the task handle, then clear busy, and only then self-delete. Clearing busy before clearing the handle lets a new start overwrite `gTask` and then have the old task null it.
- SdFat 2.3.1 directory cursors are 32-byte directory slots on both FAT (`FatFile::openNext`) and exFAT (`ExFatFile::openNext`). A saved cursor must be 32-byte aligned and must point at the entry not yet consumed. `getName` returns 0 and writes an empty string on failure or truncation; do not treat that as a short name. A host shim that uses dense item indexes will not catch a misaligned resume.
