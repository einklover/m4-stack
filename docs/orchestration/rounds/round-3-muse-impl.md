...
- Verified via `grep`: no `drawTabBar`, no `drawList`, no legacy SD-card OTA activity/updater symbols, no `selectedSettingIndex`, no `categoryCount` in new files; `drawButtonHints` single overlay, `switchBootSlot` kept, `readerLayout` present, Hub↔L2 wiring via `murphy_settings_*_m4theme.h` + `SettingsHubPolicy`/`SettingsSceneModel`.

## Visual follow-up (human feedback 2026-08-31, QEMU proved at 35859a0)
- `themes/murphy-settings/hub.json` — Hub type enlarged L1 only: page title `[24,20,320,32]` `ui_20_bold` (was 16), category `[24,32,380,36]` `ui_24_bold` (was 20) centered in row; `item_height` 140→100 gap 12→8 limit4 kept, selected bar scaled to `[0,12,4,76]` r0 fill no stroke, no icon/cover/progress/stroke/footer. Recompiled via `cd firmware && /usr/bin/python3 tools/compile_home_theme.py --theme ../themes/murphy-settings/hub.json --out /tmp/murphy_settings_hub.m4theme --emit-header src/generated/murphy_settings_hub_m4theme.h` (BBF4ADC3).
- `firmware/src/util/M4TouchNavigation.cpp` `drawBottomBar`: removed chevron/house/hamburger pixel icons, kept 1px top hairline + two vertical splits, three equal cells centered text only `返回 | 主页 | 历史` via `M4UiText::drawCenteredInBox` `UI_14` bold 12, no `UIIcon`.
- `firmware/src/util/M4TouchNavigation.h` added `hitHistory(x,y,w,h)` hitting `layout.menu` (kept Rect name, label 历史).
- `firmware/src/MappedInputManager.h/.cpp` added `wasHistoryGesture()` tapping third cell when `BottomBackHome`, `wasMenuGesture` untouched (reader top-edge swipe only).
- `firmware/src/main.cpp` gesture block next to `wasHomeGesture`: `else if (!isHome && wasHistoryGesture()) { if (dynamic_cast<RecentBooksActivity*>(currentActivity)==nullptr) onGoToRecentBooks(); return; }` — bottom bar 历史 = RecentBooks, not Menu. Forbidden `FengyanTheme::drawTabBar/drawList/drawButtonHints` (Home 3-col path) and I18n `kBack` untouched, literals in drawBottomBar.
