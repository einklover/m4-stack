...
- `firmware/src/activities/apps/AppListActivity.cpp:157-173` `displayTaskLoop` reads `updateRequired_`/`subActivity` under `renderingMutex_` every 10 ms; render-consumed main-thread writes live at many more sites than `selectIndex`/`moveSelection` (`:271-284`, unlocked): `mode_` at Back-exit `:367`, dialog-cancel `:388`, touch-footer `:425,:436`, Left-key `:464`; `uninstallClearData_` toggles at `:395,:406`; `updateRequired_` at `uninstallSelected` `:353`, sub-activity reload `:360`, and beside every write above (`:369,:390,:396,:407,:427,:438,:466`). Already safe: `reload()` `:233-241` (takes the lock, resets `mode_=0`) and `onEnter` `:248` (`updateRequired_=true` before `xTaskCreate`, no task exists yet). `M4FooterTouchPolicy::setMask` is a plain static store (`M4FooterTouchPolicy.h:15-19`, no lock interaction), so keeping each call inside the new lock scopes preserves behavior exactly. `render()` reads all three at `:490,:500,:530`; `:316-345` `openSelected`/`openInstall`/`uninstallSelected` take the mutex and `openSelected`/`openInstall` call `enterNewActivity` while holding it; `:356+` `loop()` dispatches keys/touch/swipe; `:483-535` `render()` has the single submit `:535`, called only from `displayTaskLoop`. Header `AppListActivity.h:38-41` (`touchFooterButtonsMask` reads `mode_`/`selectedIsPlugin()`), `:60-78` (`DrawerItem` with `std::string`+`pluginIcon`, `apps_`/`items_` vectors, `selectedIndex_`, `updateRequired_`, `mode_`, `uninstallClearData_`, task handle, `renderingMutex_`). Grid constants `:34-35` (`kAppLongPressMs = 700`, `kDrawerColumns = 3`).
...
- Per-site navigation regions: AppList grid release edges `AppListActivity.cpp:460-478` (Up/Down ±`kDrawerColumns`, Left uninstall-dialog plugin-gated, Right install, Confirm open, Back back); MyLibrary linear file-list navigation `MyLibraryActivity.cpp:900-955` (`upReleased` Up-or-Left, `downReleased` Down-or-Right, page (`SKIP_PAGE_MS` hold) vs row math on `selectorIndex`, `updateRequired`). Explicitly excluded from migration: `MyLibraryActivity.cpp:680-760` preview/action-menu/exit rows (preview-image menu `:686-700`, preview exit `:702-730`, Back-held `GO_HOME_MS` long-press `:732-739`, action menu `:740-756`) — their dispatch semantics stay exactly as today; Home press edges `HomeActivity.cpp:810-834` (Up-or-Left previous / Down-or-Right next pressed, Confirm released, Back pressed early-return); Settings exact enumeration — Back pressed `:194`, Confirm released `:291`, Hub pane `:303-316` (up||left → `settingsNavMoveHub(-1)`, down||right → `settingsNavMoveHub(+1)`), Row pane `:317-330` (up||left → `settingsNavMoveRow(-1)` + `settingsNavSyncWindow`, down||right → `(+1)`). These 6 references are the complete `Button::` enumeration in `SettingsActivity.cpp`; all other `settings/*` activities stay unmigrated in Phase 1.
...
- Produces: failing contract pinning (a) `M4RenderGuard.h` exists with move-forbidden RAII + explicit `unlock` + static `peek` + bounded-wait acquire (no `portMAX_DELAY` on new paths); (b) `main.cpp` owns one process-wide mutex alongside `currentActivity`; (c) ALL render-consumed main-thread writes take the local mutex (or guarded setters): `selectIndex`/`moveSelection` (`:271-284`) plus `mode_` writes (`:367,:388,:425,:436,:464`), `uninstallClearData_` toggles (`:395,:406`), and `updateRequired_` writes (`:353,:360,:369,:390,:396,:407,:427,:438,:466`) — with `setMask` policy calls preserved at each site; (d) `displayTaskLoop` implements snapshot-under-local → release → render-plus-submit-under-global with `updateRequired_`/`subActivity` rechecked under local; (e) `openSelected`/`openInstall` release the local mutex before `enterNewActivity`; (f) per-activity submit-site guard lists for AppList/Home/MyLibrary (see Task 2/8).
...
- [ ] **Step 2: RED commit and verify failure**
  ...
  Expected: fails (missing `M4RenderGuard.h`, unlocked `selectIndex`/`mode_`/`uninstallClearData_`/`updateRequired_` writes, single-phase `displayTaskLoop`, enter-under-lock in `openSelected`/`openInstall`). Non-zero exit is the RED gate — it fails because the guard, the all-writes locking, the two-phase pattern, and the release-before-enter do not exist yet.
...
- [ ] **Step 3: Minimal AppList adoption (highest traffic + confirmed race only).** Take the local mutex in `selectIndex` (`:271`) / `moveSelection` (`:278`) AND at every other render-consumed main-thread write site — `mode_` (`:367,:388,:425,:436,:464`), `uninstallClearData_` (`:395,:406`), `updateRequired_` (`:353,:360,:369,:390,:396,:407,:427,:438,:466`) — via short lock scopes or guarded setters; keep all math identical and keep every `setMask` policy call at its site (plain static store, no lock interaction, so behavior is preserved exactly). `reload()` `:233-241` already locks and `onEnter` `:248` is pre-task — leave both as-is. Rework `displayTaskLoop` (`:160-173`): take local, copy snapshot + recheck `updateRequired_ && !subActivity` (keep the existing stale-observation comment), clear `updateRequired_`, release local, then take the global guard and render-from-snapshot plus submit (`render():535`, the sole AppList submit). Rewrite `openSelected` (`:316-335`) / `openInstall` (`:336-343`): copy the needed item/app data to locals under the local mutex, release it, then `enterNewActivity` — never hold local across child installation, so no local→global nesting is possible.
...
- Modify: `firmware/src/activities/Activity.h` (add `virtual bool firstPaintComplete() const { return true; }` default — RTTI-free, null-safe: any destination without an owning task never blocks boot)
...
- Modify: `firmware/src/activities/reader/XtcReaderActivity.h` + `XtcReaderActivity.cpp` (same flag set after the first submit in `displayTaskLoop():295+` via `renderScreen():308+`)
...
- Produces (replaces the unimplementable "run its render synchronously on the main task" reading with the spec's second alternative — task-gated first paint — while keeping the primitive, timeout, assertions, and diagnostics the spec pins):
  ...
  Misuse (CrossMux-identical intent, adapted to per-activity tasks): never from any display task and never while holding the global guard — debug assert via `M4RenderGuard::peek` + setup-only call-site restriction; release builds emit the serial diagnostic and return `false` immediately. Boot sequence: enter initial activity (starts the owning task synchronously in `onEnter`) → `waitForFirstPaint()` → two-sample settle → `waitForPowerRelease()` → loop. Flag acquisition mechanism: boot calls `currentActivity->firstPaintComplete()` directly — no casts (RTTI-free), no null checks beyond the existing `currentActivity` validity: the base default returns `true` so unflagged destinations never block, `HomeActivity` overrides with its promoted atomic flag, and `ReaderActivity` overrides with the forwarding accessor over its current child. Exactly one context submits the first frame (the owning task); the timed-out wait is abandoned, never retried alongside a new submitter. No per-frame caller, no global dispatch flag, no central ActivityManager, no new public render method on any activity.

- [ ] **Step 1: Add the base default + first-paint-complete flags to the six owning activities + Home/Reader overrides.** Flags are set-once (first guarded submit only), read with acquire ordering, cleared on `onExit`. Base `Activity.h` gains only the default-`true` accessor — no render entry, no dispatch state.

- [ ] **Step 2: Add the single boot call site in `main.cpp`.** After the home-or-reader branch (`:1250-1269`), before `waitForPowerRelease()`: poll `currentActivity->firstPaintComplete()` until true or 2 s. Success emits `[MAIN] First paint wait ok`; timeout emits `[MAIN] First paint wait timeout, proceed to settle` and proceeds. Two expected-timeout cases (not failures): synchronous-path destinations whose first submit lands in early `loop()` frames, and provider-book boots where `ReaderActivity::onEnter` launches a plugin-backed `AppRuntimeActivity` whose first paint legitimately exceeds 2 s (cold plugin load) — in both, settle still guarantees silent edges because level persists while held and edges clear before first dispatch, and the owning task keeps sole ownership throughout.
...
- [ ] **Step 4: GREEN verification + commit**
  ...
  Expected: all pass. Then `git commit -m "feat(phase1-i1): first-paint completion wait + two-sample settle"` — reverting deletes the base default, the flags, the single boot call site, and the settle step; boot returns to enter-then-wait-for-release with no other item touched. QEMU boot-log proof is Task 11.
...
**Interfaces:**
- Consumes: `MappedInputManager.h:10` enum, `:177-238` getters + synthetic/footer branches, `ButtonNavigator.h:47-52`, per-site regions (AppList `:460-478`, MyLibrary linear `:900-955` with `:680-760` explicitly excluded, Home `:810-834`, Settings `:194,:291,:303-330`).
...
- [ ] **Step 2: Write the failing adoption source contract** — enum append, recursion placement, singleton getters, AppList explicit-axis verticals (never logical for Left/Right), MyLibrary linear `:900-955` conversion plus `:680-760` exclusion asserted unchanged, Home/Settings rows per Task 6–7 table.
...
- Produces:
  ...
  // Symmetric wasReleased sketch — same recursion-first order; the footer-tap
  // branch keeps its exact Back/Confirm/Left/Right eligibility and position
  // (after synth matching, before mapButton), so footer taps on member slots
  // still read through the logical query via the recursion above it:
  bool MappedInputManager::wasReleased(Button b) const {
    if (b == Button::NavNext) return wasReleased(Button::Down) || wasReleased(Button::Right);
    if (b == Button::NavPrevious) return wasReleased(Button::Up) || wasReleased(Button::Left);
    if (b == Button::Back && syntheticBack) return true;
    if (synthKind_ == SynthKind::Key && m4SynthSatisfiesKey((M4NavButton)synthKey_, (M4NavButton)b)) return true;
    ...footer-tap branch (unchanged eligibility: Back/Confirm/Left/Right only)...
    return mapButton(b, &HalGPIO::wasReleased);
  }
  ```
...
- [ ] **Step 2: Wire recursion + `m4SynthSatisfiesKey` into all three getters** in the exact sketched order above (`wasPressed` and the symmetric `wasReleased`). Footer-tap eligibility/position and synthetic-Back paths untouched.
...
- Modify: `firmware/src/activities/home/MyLibraryActivity.cpp:900-955` (linear file-list rows only: `upReleased`/`downReleased` ORs + page/row math; the `:680-760` preview/action-menu/exit rows are NOT touched — preview-image menu, preview exit, Back-held `GO_HOME_MS`, and action-menu dispatch stay exactly as today)
...
- Produces, in migration order (AppList already done in Task 6):
  - MyLibrary (second): linear file-list rows (`:900-955`) take `onNextRelease` / `onPreviousRelease` logical, preserving the `SKIP_PAGE_MS` page-vs-row math. Edge kind (release) preserved; identical behavior under the portrait policy. The `:680-760` preview/action-menu/exit rows keep their current hand-rolled dispatch (verified unchanged by contract).
...
- [ ] **Step 1: MyLibrary snapshot + submit-site guards + install-path hygiene.** Apply the same release-before-enter rewrite to the two `AppInstallActivity` install paths (`:486-492` `.m4x` tap path, `:859-865` action path: both take the local mutex across `exitActivity`+`enterNewActivity` today) — copy `fullPath` to a local under the lock, release, then exit/enter. Commit: `git commit -m "feat(phase1-r1): MyLibrary render guard"`.
...
- [ ] **Step 2: QEMU/m4adb regression (unchanged suites + boot-order proof)**
  ...
  Expected: green throughout; QEMU UART log shows guarded first paint → `[MAIN] First paint wait ok` (or the timeout marker — expected and safe on synchronous-path boots and on provider-book boots where a plugin-backed `AppRuntimeActivity` cold-loads past 2 s) → `[MAIN] Input settle done` → power-release wait, with only those markers added; MyLibrary down/confirm/back journey green without timing adjustments; long-press Back (home, 1.5 s) journey green with threshold unchanged; AppList grid vertical ±1 row and Left/Right install/uninstall bindings intact; synthetic one-shot semantics unchanged.
...
- Placeholders: none — every task names exact files, line regions, interfaces, commands, expected exits, and commit messages. Settings rows are exactly enumerated (S1 `:303-316`, S2 `:317-330`, Back `:194`, Confirm `:291`); no "remaining settings rows" language remains. Task 4 names all 14 files required to gate first paint (`Activity.h` + `main.cpp` + 6 owning-activity h/cpp pairs + Reader forwarding h/cpp).
