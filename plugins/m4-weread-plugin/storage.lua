...
-- TOC v2 includes a reliable wordCount field. Missing/older versions are
-- treated conservatively when opened offline.
Storage.TOC_VERSION = 2
...
function Storage.save_toc(bookId, chapters)
  return Storage.write_json("cache/" .. bookId .. "/toc.json", {
    version = Storage.TOC_VERSION,
    chapters = chapters,
  })
end

function Storage.load_toc(bookId)
  local doc = Storage.read_json("cache/" .. bookId .. "/toc.json")
  if doc and type(doc.chapters) == "table" then
    for i = #doc.chapters, 4097, -1 do doc.chapters[i] = nil end
    return doc.chapters, tonumber(doc.version) or 0
  end
  return nil
end
...
function Storage.save_catalog_meta(bookId, spec)
  ...
  return Storage.write_json("cache/" .. tostring(bookId) .. "/toc_catalog.json", {
    ...
    word_count_field = tonumber(spec.word_count_field) or -1,
  })
end
...
function Storage.chapter_path(bookId, chapterUid)
  return "cache/" .. bookId .. "/ch_" .. chapterUid .. ".txt"
end

function Storage.pidx_path(bookId, chapterUid)
  return "cache/" .. bookId .. "/ch_" .. chapterUid .. ".pidx"
end
