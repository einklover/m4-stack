...
-- True when chapter body is fully ready (complete marker + non-empty file).
function Storage.chapter_body_ready(bookId, chapterUid)
  return Storage.chapter_complete(bookId, chapterUid)
end
...
-- Local shelf (books the user opened; no cloud account for jjwxc).
...
function Storage.toc_ok_path(bookId)
  return Storage.toc_rows_path(bookId) .. ".ok"
end

-- Count lines in a FileRows catalog file without decoding JSON.
function Storage.count_file_lines(relPath)
  if type(relPath) ~= "string" or relPath == "" then return 0 end
  if type(fs) ~= "table" or type(fs.fileSize) ~= "function" then return 0 end
  local total = tonumber(fs.fileSize(relPath) or 0) or 0
  if total < 1 then return 0 end
  local n = 0
  if type(fs.readRange) == "function" then
    local off, chunk = 0, 4096
    while off < total do
      local want = chunk
      if want > total - off then want = total - off end
      local s = fs.readRange(relPath, off, want)
      if type(s) ~= "string" or s == "" then break end
      for _ in s:gmatch("\n") do n = n + 1 end
      off = off + #s
      if off >= total and s:sub(-1) ~= "\n" and #s > 0 then n = n + 1 end
    end
  elseif type(fs.readFile) == "function" then
    local raw = fs.readFile(relPath)
...
function Storage.toc_complete(bookId)
  if type(fs) ~= "table" or type(fs.fileSize) ~= "function" then return false end
  local okp = Storage.toc_ok_path(bookId)
  local okSz = tonumber(fs.fileSize(okp) or 0) or 0
  if okSz < 1 then return false end
  -- 只认 .ok 标记；勿在这里 count_file_lines（大书会卡 UI）
  local rows = Storage.toc_rows_path(bookId)
  local rsz = tonumber(fs.fileSize(rows) or 0) or 0
  return rsz > 0
end

function Storage.mark_toc_complete(bookId, count)
  if type(fs) ~= "table" or type(fs.writeFile) ~= "function" then return false end
  pcall(fs.writeFile, Storage.toc_ok_path(bookId), "1")
  if count and tonumber(count) and tonumber(count) > 0 then
    Storage.save_catalog_meta(bookId, {
      source = Storage.toc_rows_path(bookId),
...
-- 宿主沙箱路径只允许 [a-zA-Z0-9_-+.]; resume/进度可能带 m4cp:/绝对路径等
-- 非法字符, 清洗后再拼路径 (晋江 bookId/chapterUid 均为纯数字)。
local function safe_seg(v)
  return tostring(v or ""):gsub("[^%w_%-%+%.]", "")
end
Storage.safe_seg = safe_seg

function Storage.chapter_path(bookId, chapterUid)
  return "cache/" .. safe_seg(bookId) .. "/ch_" .. safe_seg(chapterUid) .. ".txt"
end
...
function Storage.pidx_path(bookId, chapterUid)
  return "cache/" .. safe_seg(bookId) .. "/ch_" .. safe_seg(chapterUid) .. ".pidx"
end

-- Complete chapter only: body present + .ok marker (progressive finish / atomic write).
-- Partial early-open leftovers without .ok must not be treated as cache hits
-- (avoids opening previous book's half-written chapter text).
function Storage.chapter_complete(bookId, chapterUid)
  if not bookId or tostring(bookId) == "" then return false end
  if not chapterUid or tostring(chapterUid) == "" then return false end
  if type(fs) ~= "table" or type(fs.fileSize) ~= "function" then return false end
  local n = tonumber(fs.fileSize(Storage.chapter_path(bookId, chapterUid)) or 0) or 0
  if n < 1 then return false end
  local ok = tonumber(fs.fileSize(Storage.chapter_ok_path(bookId, chapterUid)) or 0) or 0
  return ok > 0
end
...
function Storage.mark_chapter_complete(bookId, chapterUid)
  ...
  return fs.writeFile(p, "1") and true or false
end
...
function Storage.clear_chapter_cache(bookId, chapterUid)
  ...
  local okp = Storage.chapter_ok_path(bookId, chapterUid)
  if type(fs.remove) == "function" then
    pcall(fs.remove, path)
    pcall(fs.remove, okp)
  elseif type(fs.writeFile) == "function" then
    pcall(fs.writeFile, path, "")
    pcall(fs.writeFile, okp, "")
  end
end
...
function Storage.save_chapter_text(bookId, chapterUid, text)
  ...
  local ok = fs.replaceFile(path, text)
...
function Storage.chapter_ready(bookId, chapterUid)
  if Storage.chapter_complete then
    return Storage.chapter_complete(bookId, chapterUid)
  end
