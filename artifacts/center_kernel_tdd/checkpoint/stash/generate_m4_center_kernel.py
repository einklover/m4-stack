#!/usr/bin/env python3
"""Generate the reader-body 16x16 absolute-center occupancy blob (CJK only).

Canonical stored glyph: absolute-position 16x16 center occupancy + 2-bit joint
class from 标准像素粗.ttf contours (analytic point-in-contour), matching
pixel_center_absolute_report.json. Do not left-normalize by bbox/LSB.

Chrome continues to use the separate native-grid 15x16 asset. This blob is the
reader-body CJK corpus only (U+3400–U+9FFF mapped glyphs).
...
def collect_corpus(font_path: Path):
    ...
    mapped = {cp: name for cp, name in cmap.items() if CJK_LO <= cp <= CJK_HI}
...
def verify_tracked_assets(blob_path: Path, header_path: Path, manifest_path: Path) -> int:
    ...
    if manifest.get("supported_count") != EXPECTED_GLYPHS:
...
def main() -> int:
    ...
    if metadata["supported_count"] != EXPECTED_GLYPHS:
        raise SystemExit(f"glyph count {metadata['supported_count']} != {EXPECTED_GLYPHS}")
    expected_hist = {"0": 18376, "1": 9172, "2": 4, "3": 1}
    if metadata["joint_class_histogram"] != expected_hist:
        raise SystemExit(f"class hist {metadata['joint_class_histogram']} != {expected_hist}")
